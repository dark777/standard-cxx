...
namespace osiris
{
	public class validacion_cargos_extras
	{
		...
		public validacion_cargos_extras(string folio_)
		{
		...
		void on_button_calcular_clicked(object sender, EventArgs args)
		{
			...
			if (radiobutton_paquete.Active == true){
				num_presu_paquete_folio = "id_tipo_cirugia = '"+entry_paquete.Text.Trim()+"' ";
			...
			try{
				...
				comando.CommandText = comando.CommandText = "SELECT folio_de_servicio,to_char(osiris_erp_cobros_deta.id_producto,'999999999999') AS idproducto,osiris_productos.descripcion_producto,"+
									...
									"WHERE folio_de_servicio = '"+folioservicio.Trim()+"' "+
				...
				while(lector.Read()){
					...
					toma_totales =  decimal.Parse((string) lector["cantidadaplicada"]);
					...
					try{
						...
						if(lector1.Read()){							
							...
							calculadiferencia = toma_totales - decimal.Parse((string) lector1["cantidadaplicada"]);							
							if(calculadiferencia != 0){							
								if (toma_totales != decimal.Parse((string) lector1["cantidadaplicada"])){
							...
							treeViewEngineCargos.AppendValues (idproducto_tabla,
															toma_descripcion,
															toma_totales.ToString("F"),"(Extra) No esta en el Paquete"," ",iva.ToString("F"),total.ToString("F"));
						}	
			...
			try{
				...
				while(lector2.Read()){
					...
					toma_totales = decimal.Parse((string) lector2["cantidadaplicada"]);
					...
					if((bool) lector2["aplicar_iva"] == true){
						iva = 0;
						iva = Convert.ToDecimal((Convert.ToDouble((string) lector2["precioproducto"]) * 1.16) - Convert.ToDouble((string) lector2["precioproducto"]));
