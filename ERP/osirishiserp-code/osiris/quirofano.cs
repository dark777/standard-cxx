...
namespace osiris
{
	public class quirofano
	{
		...
		[Widget] Gtk.Button button_caducos = null;
		...
		public quirofano (string LoginEmp_, string NomEmpleado_, string AppEmpleado_, string ApmEmpleado_, string nombrebd_,bool accesocxpq_) 
		{
			...
			//button_caducos.Clicked += new EventHandler(on_button_caducos_clicked);
			//button_salir.Clicked += new EventHandler(on_cierraventanas_clicked);
		}
		...
		{
			if (accesocxpq == true ){
 				new osiris.paquetes_cirugias (LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,"1");
			}else{
		...
		{
			new osiris.solicitud_material(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,5,0,0,"");
		}
		...
		{
			new osiris.inventario_sub_almacen(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,5,"QUIROFANO",3,false);
		}
		...
		{
			new osiris.trasp_devolu_materiales(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,5,"QUIROFANO",1,"DEVOLUCION",false,1);
		}
		...
		void imprime_reporte_cirugias(object sender, EventArgs args)
		{
			...
			if(checkbutton_export_to.Active == true){
				string query_sql = "SELECT DISTINCT ON (osiris_erp_pases_qxurg.folio_de_servicio) osiris_erp_pases_qxurg.folio_de_servicio, osiris_erp_pases_qxurg.id_secuencia AS nro_pase,osiris_erp_pases_qxurg.folio_de_servicio AS foliodeservicio,to_char(osiris_erp_cobros_enca.pid_paciente,'9999999999') AS pidpaciente," +
						...
						"to_char(age(osiris_erp_pases_qxurg.fechahora_creacion,osiris_his_paciente.fecha_nacimiento_paciente),'YY') AS edadpx," +
						...
						"to_char(osiris_erp_pases_qxurg.fechahora_creacion,'yyyy-MM-dd') AS fechapaseqx,osiris_erp_pases_qxurg.id_tipo_admisiones,osiris_erp_cobros_enca.id_empresa AS idempresa,osiris_empresas.descripcion_empresa," +
					...
					"FROM osiris_erp_pases_qxurg,osiris_his_tipo_admisiones,osiris_erp_cobros_enca,osiris_his_paciente,osiris_empleado,osiris_empresas,osiris_aseguradoras,osiris_erp_movcargos,osiris_his_tipo_pacientes,osiris_his_tipo_cirugias,osiris_his_medicos "+
						"WHERE osiris_erp_pases_qxurg.pid_paciente = osiris_his_paciente.pid_paciente " +
						...
						string[] args_names_field = {"fechapaseqx","nro_pase","tipodepase","foliodeservicio","pidpaciente","nombre_completo","edadpx","motivo_ingreso","descripcion_tipo_paciente","dr_solicita","medicotratante","cerrado",
													"idproducto","descripcion_producto","numero_serie","tipoanestesia","idcirujano","nombre_cirujano","idanestesiologo","nombre_anestesiologo","id_ayudante","nombre_ayudante","cxpaseqx","observacionespaseqx",
													"eliminado","motivo_eliminacion","observaciones1"};
				string[] args_type_field = {"string","float","string","float","float","string","string","string","string","string","string","string","string","string","string","string","float","string","float","string","float","string","string","string","string","string","string"};
				string[] args_field_text = {""};
				string[] args_more_title = {""};
				string[,] args_formulas = {{"",""}};
				string[,] args_width = {{"2","5cm"},{"5","7cm"},{"6","1cm"},{"7","7cm"},{"8","5cm"},{"9","6cm"},{"10","6cm"}};
