...
// Monterrey - Mexico
//
// Autor    	: Ing. Daniel Olivares - arcangeldoc@openmailbox.org (Diseño de Pantallas Glade)
// 				  Ing. Jesus Buentello Garza (Programacion y reporte)
//				  Ing. Juan Antonio Peña (Ajustes y Programacion)
//
// Licencia		: GLP
// S.O. 		: GNU/Linux
//////////////////////////////////////////////////////////
...
namespace osiris
{
	public class envio_de_materiales_subalmacenes
	{
		...
		[Widget] Gtk.Window envia_materiales_subalmacen = null;
		[Widget] Gtk.TreeView lista_de_materiales_solicitados = null;
		[Widget] Gtk.ComboBox combobox_sub_almacenes = null;
		[Widget] Gtk.ComboBox combobox_almacen_origen = null;
		...
		[Widget] Gtk.Button button_sin_stock = null;
		[Widget] Gtk.Button button_pedido_erroneo = null;
		[Widget] Gtk.Button button_no_surtir = null;
		...
		[Widget] Gtk.Button button_rpt_surtido = null;
		...
		[Widget] Gtk.CheckButton checkbutton_refresh_query = null;
		[Widget] Gtk.CheckButton checkbutton_filtro_soli = null;
		[Widget] Gtk.Entry entry_numero_solicitud = null;
		[Widget] Gtk.Button button_filtrosolicitud = null;
		[Widget] Gtk.Button button_imprime_solic = null;
		[Widget] Gtk.CheckButton checkbutton_envio_directo = null;
		[Widget] Gtk.Entry entry_codigobarras = null;
		
		[Widget] Gtk.Button button_actualiza_query = null;
		...
		TreeViewColumn col_envios19;		CellRendererText cellrt19;
		...
		[Widget] Gtk.ProgressBar progressbar_robot_query = null;
		struct ProgressData {
		...
		bool progress_timeout()
		{
			...
			if (pdata.activity_mode){
				progressbar_robot_query.Pulse();
				//Console.Write("ejecutando la consulta QuerySql "+progressbar_robot_query.Fraction.ToString()+"\n");
				llenado_de_material_solicitado();
			}else {
				/* Calculate the value of the progress bar using the value range set in the adjustment object */
				new_val = progressbar_robot_query.Fraction + 0.01;
				if (new_val > 1.0)
					new_val = 0.0;	 
				/* Set the new value */
				progressbar_robot_query.Fraction = new_val;
			}	 
		...
		bool progress_timeoff()
		{
			...
			/* As this is a timeout function, return TRUE so that it continues to get called */
	 		if(pdata.stop_progressData == true){
				...
				progressbar_robot_query.Fraction = 0.0;
				return false;
			}
		}
		
		void on_checkbutton_refresh_query_clicked (object obj, EventArgs args)
		{
		...
		public envio_de_materiales_subalmacenes(string LoginEmp_, string NomEmpleado_, string AppEmpleado_, string ApmEmpleado_, string nombrebd_) 
		{
			...
			entry_numero_solicitud.KeyPressEvent += onKeyPressEvent;			
			...
			button_no_surtir.Clicked += new EventHandler(on_button_no_surtir_clicked);
			...
			button_actualiza_query.Clicked += new EventHandler(on_button_actualiza_query_clicked);
			button_filtrosolicitud.Clicked += new EventHandler(on_button_actualiza_query_clicked);
			...
			button_imprime_solic.Clicked += new EventHandler(on_button_imprime_solic_clicked);
			// Radiobutton
			...
			checkbutton_refresh_query.Clicked += new EventHandler(on_checkbutton_refresh_query_clicked);
			checkbutton_filtro_soli.Clicked += new EventHandler(on_checkbutton_filtro_soli_clicked);
			...
			button_filtrosolicitud.Sensitive = false;
			button_imprime_solic.Sensitive = false;
			entry_codigobarras.Sensitive = false;
			...
			try{
				...
				comando = conexion.CreateCommand ();
               	comando.CommandText = "SELECT id_almacen,descripcion_almacen,sub_almacen,almacen_salidas,cargo_directo " +
		...
		void on_button_actualiza_query_clicked(object obj, EventArgs args)
		{
			llenado_de_material_solicitado();
		}
		...
		void on_checkbutton_filtro_soli_clicked(object sender, EventArgs args)
		{
			entry_numero_solicitud.Sensitive = (bool) checkbutton_filtro_soli.Active;
			button_filtrosolicitud.Sensitive = (bool) checkbutton_filtro_soli.Active;
			button_imprime_solic.Sensitive = (bool) checkbutton_filtro_soli.Active;
			entry_codigobarras.Sensitive = (bool) checkbutton_filtro_soli.Active;
			if(checkbutton_filtro_soli.Active == false){
				llenado_de_material_solicitado();
			}
		}
		...
		void on_button_rpt_surtido_clicked(object sender, EventArgs args)
		{
			...
			new osiris.rpt_envio_almacen(nombrebd,idsubalmacen);
		}

		void on_button_imprime_solic_clicked(object sender, EventArgs args)
		{
			string query_in_num = " AND osiris_his_solicitudes_deta.folio_de_solicitud IN ('"+entry_numero_solicitud.Text.Trim()+"') ";
			string query_in_almacen = " AND osiris_his_solicitudes_deta.id_almacen IN ('"+idsubalmacen.ToString().Trim()+"') ";
			string query_fechas = " ";
		...
		void on_button_surtir_materiales_clicked(object sender, EventArgs args)
		{
			string[,] parametros;
			object[] paraobj;
			bool valida_cant_cero = false;
			string nroatencion = "0";
			string nroexpediente = "0";
						...
						if (miResultado == ResponseType.Yes){
							if ((bool) lista_de_materiales_solicitados.Model.GetValue (iterSelected,0) == true){								
								if (checkbutton_envio_directo.Active == true) {
									valida_cant_cero = true;
								} else {
									if (decimal.Parse ((string)lista_de_materiales_solicitados.Model.GetValue (iterSelected, 2)) > 0) {
										valida_cant_cero = true;
										if ((string)classpublic.lee_registro_de_tabla ("osiris_his_solicitudes_deta", "id_secuencia", " WHERE id_secuencia = '" + lista_de_materiales_solicitados.Model.GetValue (iterSelected, 7) + "' AND surtido = 'false' ", "id_secuencia", "string") != "") {
											valida_cant_cero = true;
										} else {
											valida_cant_cero = false;
										}
									} else {
										valida_cant_cero = false;
									}
								}
								if (valida_cant_cero == true) {
									// realiza un cargo automatico en la tabla de cobros deta
									if ((string)lista_de_materiales_solicitados.Model.GetValue (iterSelected, 10) != "0") {
										if ((bool)cargo_automatico == true && (bool)checkbutton_envio_directo.Active == false) {
											// si es envio directo crea una solicitud como envio directo en la tabla de solicitudes DETA
											if ((string)classpublic.lee_registro_de_tabla ("osiris_his_solicitudes_deta", "id_secuencia", " WHERE id_secuencia = '" + lista_de_materiales_solicitados.Model.GetValue (iterSelected, 7) + "' AND surtido = 'false' ", "id_secuencia", "string") != "") {
												if (radiobutton_pre_solic.Active == true) {
													nroatencion = entry_folio_servicio.Text.Trim ();
													nroexpediente = entry_pid_paciente.Text.Trim ();
												} else {
													nroatencion = lista_de_materiales_solicitados.Model.GetValue (iterSelected,10).ToString().Trim();
													nroexpediente = lista_de_materiales_solicitados.Model.GetValue (iterSelected,11).ToString().Trim();
												}
												parametros = new string[,] { 
															{"id_producto,","'" + lista_de_materiales_solicitados.Model.GetValue (iterSelected, 5).ToString ().Trim () + "',"}, 
															{"folio_de_servicio,","'"+nroatencion+"',"},
															{"pid_paciente,","'"+nroexpediente+"',"},						
															{"cantidad_aplicada,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,2).ToString().Trim()+"',"},	 														
															{"id_tipo_admisiones,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,16).ToString().Trim()+"',"},						
															{"precio_costo_unitario,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,17).ToString().Trim()+"',"},
															{"precio_producto, ","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,19).ToString().Trim()+"',"},
															{"iva_producto,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,20).ToString().Trim()+"',"},
															{"porcentage_utilidad,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,21).ToString().Trim()+"',"},
															{"porcentage_descuento,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,22).ToString().Trim()+"',"},
															{"precio_costo,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,23).ToString().Trim()+"',"},
												...
												paraobj = new object[] { entry_numero_solicitud};
												new osiris.insert_registro ("osiris_erp_cobros_deta", parametros, paraobj);
											}
										}
									}
									// verificando el producto si esta en el sub-almacen donde se enviara sino crea el producto en en el catalogo
									if ((string)classpublic.lee_registro_de_tabla ("osiris_catalogo_almacenes", "id_producto", " WHERE osiris_catalogo_almacenes.id_almacen = '" + idsubalmacen.ToString () + "' AND eliminado = 'false' AND id_producto = '" + (string)lista_de_materiales_solicitados.Model.GetValue (iterSelected, 5) + "' ", "id_producto", "string") == "") {
										parametros = new string[,] {
																{ "id_almacen,", "'" + idsubalmacen.ToString ().Trim () + "'," },
																{"id_producto,","'" + lista_de_materiales_solicitados.Model.GetValue (iterSelected, 5).ToString ().Trim () + "',"},
																{ "stock,", "'" + lista_de_materiales_solicitados.Model.GetValue (iterSelected, 2).ToString ().Trim () + "'," },
										...
										paraobj = new object[] { entry_numero_solicitud };
										new osiris.insert_registro ("osiris_catalogo_almacenes", parametros, paraobj);
									} else {
										// si existe en producto en el almacen de destino suma las cantidades a surtir
										parametros = new string[,] {
																{ "stock = stock + '",lista_de_materiales_solicitados.Model.GetValue (iterSelected,2).ToString().Trim()+"'," },
																{ "fechahora_ultimo_surtimiento = '",DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"' " },
																{ "WHERE eliminado = '","false' " },
																{ "AND id_almacen = '",idsubalmacen.ToString()+"' "},
																{ "AND id_producto = '", lista_de_materiales_solicitados.Model.GetValue (iterSelected,5).ToString().Trim() + "';" }
															};
										paraobj = new object[] { entry_numero_solicitud };
										new osiris.update_registro ("osiris_catalogo_almacenes", parametros, paraobj);										
									}
									// si es no es envio directo verifica que la cantidad a surtir sea mayor a cero 
									if (checkbutton_envio_directo.Active == false) {
										// verifica si es pre-solicitud
										if ((bool)lista_de_materiales_solicitados.Model.GetValue (iterSelected, 14) == true) {
											if (int.Parse ((string)entry_folio_servicio.Text.ToString ()) != 0) {
												parametros = new string[,] {
													{ "cantidad_autorizada = '",lista_de_materiales_solicitados.Model.GetValue (iterSelected,2).ToString().Trim()+"'," },
													...
													{ "surtido = '","true'," },
													{ "folio_de_servicio = '",entry_folio_servicio.Text.ToString()+"'," },
													{ "pid_paciente = '",entry_pid_paciente.Text.ToString()+"' " },
													{ "WHERE id_secuencia = '",lista_de_materiales_solicitados.Model.GetValue (iterSelected,7)+"' "},
													{ "AND surtido = '","false' "}
												};
												paraobj = new object[] { entry_numero_solicitud };
												new osiris.update_registro ("osiris_his_solicitudes_deta", parametros, paraobj);
											}
										}else{
											parametros = new string[,] {
													{ "cantidad_autorizada = '",lista_de_materiales_solicitados.Model.GetValue (iterSelected,2).ToString().Trim()+"'," },
										...
										// si es envio directo crea una solicitud como envio directo en la tabla de solicitudes DETA
										parametros = new string[,] {
												{ "folio_de_solicitud,", "'" + "0" + "'," }, 
												{ "id_producto,", "'" + lista_de_materiales_solicitados.Model.GetValue(iterSelected,5).ToString().Trim() + "'," },
												{ "precio_producto_publico,", "'" + lista_de_materiales_solicitados.Model.GetValue(iterSelected,9).ToString().Trim() + "'," },
												{ "costo_por_unidad,", "'" + lista_de_materiales_solicitados.Model.GetValue(iterSelected,8).ToString().Trim() + "'," },
												{ "cantidad_solicitada,", "'" + lista_de_materiales_solicitados.Model.GetValue(iterSelected,1).ToString().Trim() + "'," },
												{ "cantidad_autorizada,", "'" + lista_de_materiales_solicitados.Model.GetValue(iterSelected,2).ToString().Trim() + "'," },
												...
												{ "fechahora_autorizado,", "'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'," },
												{ "status,", "'" + "true" + "'," },
												{ "solicitud_stock,", "'" + checkbutton_stock_almacen.Active.ToString().Trim() + "'," },
												{ "folio_de_servicio,", "'" + entry_folio_servicio.Text.ToString().Trim() + "'," },
												{ "pid_paciente", "'" + entry_pid_paciente.Text.ToString() + "' " },												
											};
										paraobj = new object[] { entry_numero_solicitud};
										new osiris.insert_registro ("osiris_his_solicitudes_deta", parametros, paraobj);											
									}
									// descontado al almacen de origen 
									if(idsubalmacen != idalmacenorigen){
										parametros = new string[,] {
											{ "stock = stock - '",lista_de_materiales_solicitados.Model.GetValue (iterSelected,2).ToString().Trim()+"' " },
											{ "WHERE eliminado = '","false' " },
									...
									if (valida_cant_cero == true) {
										...
										if ((string)lista_de_materiales_solicitados.Model.GetValue (iterSelected, 10) != "0") {
											if ((bool)cargo_automatico == true && (bool)checkbutton_envio_directo.Active == false) {
												// si es envio directo crea una solicitud como envio directo en la tabla de solicitudes DETA
												if ((string)classpublic.lee_registro_de_tabla ("osiris_his_solicitudes_deta", "id_secuencia", " WHERE id_secuencia = '" + lista_de_materiales_solicitados.Model.GetValue (iterSelected, 7) + "' AND surtido = 'false' ", "id_secuencia", "string") != "") {
													parametros = new string[,] { {"id_producto,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,5).ToString().Trim()+"',"},
														{"folio_de_servicio,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,10).ToString().Trim()+"',"},
														{"pid_paciente,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,11).ToString().Trim()+"',"},						
														{"cantidad_aplicada,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,2).ToString().Trim()+"',"},	 														
														{"id_tipo_admisiones,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,16).ToString().Trim()+"',"},						
														{"precio_costo_unitario,","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,17).ToString().Trim()+"',"},
														{"precio_producto, ","'"+lista_de_materiales_solicitados.Model.GetValue (iterSelected,19).ToString().Trim()+"',"},
				...
				msgBox.Destroy();
		 		if (miResultado == ResponseType.Yes){
					parametros = new string[,] {
						...
						{ "fechahora_autorizado = '",DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"'," },
						{ "sin_stock = '","true' " },
						{ "WHERE id_secuencia = '",lista_de_materiales_solicitados.Model.GetValue (iterSelected,7).ToString().Trim()+"';" }
					};
					paraobj = new object[] { entry_numero_solicitud };
					new osiris.update_registro ("osiris_his_solicitudes_deta", parametros, paraobj);
					while (treeViewEngineSolicitado.IterNext(ref iterSelected)){
		...
		void on_button_no_surtir_clicked(object sender, EventArgs args)
		{
			TreeIter iterSelected;
 			if(treeViewEngineSolicitado.GetIterFirst (out iterSelected)){
 				MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
									MessageType.Question,ButtonsType.YesNo,"¿ Esta seguro de marcar NO SURTIR, todos los productos seleccionados ?");
				ResponseType miResultado = (ResponseType)msgBox.Run ();
				msgBox.Destroy();
		 		if (miResultado == ResponseType.Yes){
		 			NpgsqlConnection conexion; 
		...
		void on_button_busca_paciente_clicked(object sender, EventArgs args)
		{
			...
			string[] parametros_sql = {"SELECT osiris_erp_cobros_enca.folio_de_servicio,osiris_his_paciente.pid_paciente AS pidpaciente,nombre1_paciente,nombre2_paciente, apellido_paterno_paciente,id_quienlocreo_paciente,"+
									...
									"to_char(fechahora_registro_paciente,'dd-MM-yyyy HH:mi:ss') AS fech_creacion,activo FROM osiris_his_paciente,osiris_erp_cobros_enca "+
									"WHERE activo = 'true' "+
			...
			string[,] args_buscador1 = {{"APELLIDO PATERNO","AND apellido_paterno_paciente LIKE '%","%' "},
										...
										{"NRO. ATENCION","AND osiris_erp_cobros_enca.folio_de_servicio = '","' "},
			...
			string[,] args_buscador2 = {{"APELLIDO MATERNO","AND apellido_materno_paciente LIKE '%","%' "},
										...
										{"NRO. EXPEDIENTE","AND osiris_his_paciente.pid_paciente = '","' "}};
			string[,] args_orderby = {{"",""}};
			classfind_data.buscandor(parametros_objetos,parametros_sql,parametros_string,"find_paciente",1,args_buscador1,args_buscador2,args_orderby);
		}
			
		void llenado_de_material_solicitado()
		{
			float calculodeiva = 0;
			float preciomasiva = 0;
			float tomadescue = 0;
			float preciocondesc = 0;
			float tomaprecio = 0;
			TreeIter iter2;
			treeViewEngineSolicitado.Clear(); // Limpia el treeview cuando realiza una nueva busqueda
			string sql_nrosolicitud = "";
			if (checkbutton_filtro_soli.Active == true){
				if(entry_numero_solicitud.Text != ""){
					sql_nrosolicitud = "AND folio_de_solicitud = '"+entry_numero_solicitud.Text+"' ";
				}
			}
			...
			string sql_envio_subalmacenes = "SELECT osiris_his_solicitudes_deta.folio_de_solicitud,to_char(osiris_his_solicitudes_deta.id_producto,'999999999999') AS idproductos,"+
									...
									"to_char(osiris_productos.precio_producto_publico,'99999999.99') AS preciopublico,aplicar_iva,aplica_descuento,"+
									...
									"to_char(osiris_his_solicitudes_deta.costo_por_unidad,'999999999.99') AS costoporunidad,osiris_productos.aplicar_iva,"+
									...
									"osiris_almacenes.id_tipo_admisiones,observaciones_solicitud,descripcion_grupo2_producto "+
									"FROM osiris_his_solicitudes_deta,osiris_almacenes,osiris_his_paciente,osiris_productos,osiris_grupo_producto,osiris_grupo1_producto,osiris_grupo2_producto "+
			...
			try{
				...
				while(lector.Read()){
					...
					tomaprecio = precio_a_tomar_catalogo(lector["foliodeatencion"].ToString().Trim(),lector["idproductos"].ToString().Trim());
					...
					treeViewEngineSolicitado.AppendValues(false,
						lector["cantidadsolicitada"].ToString().Trim(),
													"0",
						lector["foliodesolicitud"].ToString().Trim(),
						lector["descripcion_producto"].ToString().Trim(),
						lector["idproductos"].ToString().Trim(),
						...
						lector["idsecuencia"].ToString().Trim(),
													...
													lector["procedimiento_qx"].ToString().Trim(),
													lector["id_tipo_admisiones"].ToString().Trim(),
													lector["costoporunidad"].ToString().Trim(),
													lector["observaciones_solicitud"].ToString().Trim(),
						tomaprecio.ToString().Trim(),
						calculodeiva.ToString("F").PadLeft(10),
						lector["porcentageutilidad"].ToString(),
					...
					col_envios19.SetCellDataFunc(cellrt19, new Gtk.TreeCellDataFunc(cambia_colores_fila));
		...
		float precio_a_tomar_catalogo(string numero_atencion_,string codigoproducto_)
		{
			string precio_a_tomar = "";
			int idempresa_paciente = 1;
			int idaseguradora_paciente = 1;
			bool aplica_precios_aseguradoras = false;
			bool aplica_precios_empresas = false;
			...
			try{
				...
				comando.CommandText = "SELECT osiris_erp_cobros_enca.folio_de_servicio AS foliodeatencion, "+
				            	...
				            	"osiris_erp_cobros_enca.pagado,monto_convenio,id_referido,osiris_erp_cobros_enca.pagare,"+
								...
								"AND osiris_erp_cobros_enca.folio_de_servicio = '"+numero_atencion_+"';";
			...
			// en esta variable dejo el precio que va tomar para los direfentes clientes
			...
			try{
				...
				comando1.CommandText = "SELECT to_char(osiris_productos.id_producto,'999999999999') AS codProducto,"+
							...
							"AND osiris_productos.id_producto ='"+codigoproducto_+"';";
				...
				while (lector1.Read()){
					...
					if (float.Parse((string) lector1["preciopublico_cliente"]) > 0){
						tomaprecio = float.Parse((string) lector1["preciopublico_cliente"]);
					}else{
						tomaprecio = float.Parse((string) lector1["preciopublico"]);
					}					
				}
			}catch (NpgsqlException ex){
	   			MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
		...
		void crea_treeview_envio_materiales()
		{
			...
			col_envios19 = new TreeViewColumn();
			cellrt19 = new CellRendererText();
			col_envios19.Title = "Precio Prod.";
			col_envios19.PackStart(cellrt19, true);
			col_envios19.AddAttribute (cellrt19, "text", 19);
			col_envios19.SortColumnId = (int) Column_solicitudes.col_envios19;
			...
			lista_de_materiales_solicitados.AppendColumn(col_envios19);
		}
		...
		enum Column_solicitudes
		{
			...
			col_envios04,
			col_envios05,
			col_envios06,col_envios10,col_envios11,col_envios12,col_envios13,
			col_envios14,col_envios15,col_envios18,col_envios19
		}
		...
		void selecciona_fila(object sender, ToggledArgs args)
		{
			...
			if (lista_de_materiales_solicitados.Model.GetIter (out iter,new TreePath (args.Path))) {
				...
				if((bool) lista_de_materiales_solicitados.Model.GetValue (iter,0) == false){
					treeViewEngineSolicitado.SetValue(iter,(int) Column_solicitudes.col_envios02,"0");
				}else{
					treeViewEngineSolicitado.SetValue(iter,(int) Column_solicitudes.col_envios02,(string) lista_de_materiales_solicitados.Model.GetValue (iter,1));
				}
			}	
		}
			...
			if((bool) checkbutton_stock_almacen.Active == true || (bool) checkbutton_stock_paciente.Active == true){
				...
				if((bool) valida_paciente == true){				
					if (lista_de_producto.Selection.GetSelected(out model, out iterSelected)){
		 				if((bool) checkbutton_envio_directo.Active == true){
							treeViewEngineSolicitado.AppendValues(true,
		...
		void NumberCellEdited_Autorizado (object o, EditedArgs args)
		{
			...
			if (esnumerico == true){		
				treeViewEngineSolicitado.SetValue(iter,(int) Column_solicitudes.col_envios02,args.NewText);
				bool old = (bool) lista_de_materiales_solicitados.Model.GetValue (iter,0);
				lista_de_materiales_solicitados.Model.SetValue(iter,0,!old);
			}
 		}
