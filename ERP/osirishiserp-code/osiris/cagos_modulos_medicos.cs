...
namespace osiris
{
	public class cargos_modulos_medicos
	{
		...
		[Widget] Gtk.Notebook notebook3 = null;
		...
		[Widget] Gtk.Button button_solicitud_optica = null;
		...
		// pestaña de la consultas medica especialidades
		[Widget] Gtk.TreeView treeview_especialidad_medica = null;
		[Widget] Gtk.Button button_consulta_medica = null;
			
		// pestaña de servicios realizados
		...
		TreeStore treeViewEngineEspeMed;
		TreeStore treeViewEngine_atenciones;
		                       ...
		                       int idtipointernamiento_,string descripinternamiento_,int idsubalmacen_,string validacion_sql) 
		{
			...
			///Console.WriteLine(idtipointernamiento.ToString());
			...
			button_solicitud_optica.Clicked += new EventHandler(on_solicitud_labrx_clicked);
			...
			button_resultados_lab.Sensitive = false;
			...
			button_asignacion_habitacion.Sensitive = false;
			button_historiaclinica.Sensitive = false;
			button_notas_evolucion.Sensitive = false;
			button_notas_enfermeria.Sensitive = false;
			button_indicacionesmedicas.Sensitive = false;
			button_documentos_medicos.Sensitive = false;
			button_pase_quirofano.Sensitive = false;
			button_solicitud_optica.Sensitive = false;
			button_solicitud_nutricion.Sensitive = false;
		...
		void on_button_paquetes_qx_clicked(object sender, EventArgs args)
		{
			...
			string[] parametros_string = {LoginEmpleado,id_tipopaciente.ToString(),idempresa_paciente.ToString(),idaseguradora_paciente.ToString(),aplica_precios_aseguradoras.ToString(),aplica_precios_empresas.ToString(),idtipointernamiento.ToString(),idsubalmacen.ToString()};
			string[,] args_buscador1 = {{"NOMBRE CIRUGIA","WHERE descripcion_cirugia LIKE '%","%' "},
										{"ID CIRUGIA","WHERE id_tipo_cirugia ='","' "}};
			string[,] args_buscador2 = {{"ID CIRUGIA","WHERE id_tipo_cirugia ='","' "},
										{"NOMBRE CIRUGIA","WHERE descripcion_cirugia LIKE '%","%' "}};
			string[,] args_orderby = {{"",""}};
			classfind_data.buscandor(parametros_objetos,parametros_sql,parametros_string,"find_cirugia_cargos_modmedicos",0,args_buscador1,args_buscador2,args_orderby);
		}
			...
			//if(button_busca_producto.Sensitive == true){
				if(button_solicitud_labrx.Name.ToString() == "button_solicitud_lab"){					
					new osiris.solicitudes_enfermeria(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,"Solicitud Estudio de LABORATORIO",400,"LAB",
					                                  ...
					                                  entry_cirugia.Text.Trim(),entry_id_habitacion.Text.Trim(),button_busca_producto.Sensitive,
				                                  	  aplica_precios_tipopx,aplica_precios_sub_tipopx,idtipointernamiento);
				}
	 					...
	 					if (miResultado == ResponseType.Yes){
							...
							try{
						        ...
						        comando.CommandText = "UPDATE osiris_catalogo_almacenes "+
													"SET stock = stock + '"+lista_de_servicios.Model.GetValue(iter,1).ToString().Trim()+"' "+
		...
		void llenado_de_productos_aplicados(string foliodeserv)
		{
			...
			try{
				...
				comando.CommandText = "SELECT DISTINCT (osiris_erp_movcargos.folio_de_servicio),osiris_erp_cobros_enca.folio_de_servicio AS foliodeatencion, "+
						        ...
						        "alta_paciente,osiris_his_medicos.id_especialidad AS idespecialidad,"+
				...
				if(lector.Read()){
					...
					button_resultados_lab.Sensitive = true;
					button_solicitud_optica.Sensitive = true;
					button_solicitud_nutricion.Sensitive = true;
					button_solicitud_lab.Sensitive = true;
					button_solicitud_rx.Sensitive = true;
					button_solicitud_vision.Sensitive = true;
					button_alta_paciente.Sensitive = true;
					button_asignacion_habitacion.Sensitive = true;
					button_historiaclinica.Sensitive = true;
					button_notas_evolucion.Sensitive = true;
					button_notas_enfermeria.Sensitive = true;
					button_indicacionesmedicas.Sensitive = true;
					button_documentos_medicos.Sensitive = true;
					...
					MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
						MessageType.Info,ButtonsType.Close,"El Numero de Atencion Seleccionado \n"+
															"NO EXISTE... verifique");
					msgBoxError.Run ();				msgBoxError.Destroy();
				}
			}catch (NpgsqlException ex){
		...
		// llena la lista de productos
 		...
 		{
 			string acceso_a_grupos = classpublic.lee_registro_de_tabla("osiris_almacenes","id_almacen"," WHERE osiris_almacenes.id_almacen = '"+idsubalmacen.ToString().Trim()+"' ","acceso_grupo_prodcargos","string");
			...
			try{
				...
				while (lector.Read()){
					...
					// las multiples listas de precio					
					...
					tomadescue = float.Parse(lector["porcentagesdesc"].ToString().Trim());
		...
		void NumberCellEdited (object o, EditedArgs args)
		{
			...
			try {
				foo = (Item) arraycargosextras[i];
 				foo.col_cantidad = float.Parse(args.NewText);
		...
		void on_button_consulta_medica_clicked(object sender, EventArgs args)
		{
			TreeModel model;
			TreeIter iterSelected;
		 	if (treeview_especialidad_medica.Selection.GetSelected(out model, out iterSelected)){			
				new osiris.notas_medicas(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,"Consulta Medica","consulta_medica",
			                         ...
			                         button_alta_paciente.Sensitive,entry_edad.Text.Trim(),entry_cirugia.Text.Trim(),
				                         (int) model.GetValue(iterSelected, 1),
				                         (string) model.GetValue(iterSelected, 0),
				                         (int) model.GetValue(iterSelected, 2),
					(int) model.GetValue(iterSelected, 3),idtipointernamiento,descripinternamiento,
				                         entry_fecha_nacimiento.Text,sexopaciente,dir_pac,entry_telefono_paciente.Text,
				                         idsubalmacen,entry_tipo_paciente.Text,entry_aseguradora.Text);

			}else{
				MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,MessageType.Info,ButtonsType.Close, "NO hay Especialidades Medicas Asignadas, verifique....");
				msgBoxError.Run ();	msgBoxError.Destroy();	
			}			
		}
		...
		void on_button_notas_evolucion_clicked(object sender, EventArgs args)
		{
			//if(button_busca_producto.Sensitive == true){
				new osiris.notas_medicas(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,"Notas de Evolucion","notas_de_evolucion",
			                         PidPaciente.ToString(),entry_folio_servicio.Text,entry_id_doctor.Text,entry_doctor.Text,entry_nombre_paciente.Text,
				button_alta_paciente.Sensitive,entry_edad.Text.Trim(),entry_cirugia.Text.Trim(),1,"",1,1,idtipointernamiento,descripinternamiento,entry_fecha_nacimiento.Text,
			                         sexopaciente,dir_pac,entry_telefono_paciente.Text,idsubalmacen,entry_tipo_paciente.Text,entry_aseguradora.Text);
			//}else{
		...
		void crea_treeview_espmed_consulta()
		{
			treeViewEngineEspeMed = new TreeStore(typeof(string),
			                                      typeof(int),
			                                      typeof(int),
			                                      typeof(int));
				
			treeview_especialidad_medica.Model = treeViewEngineEspeMed;
			treeview_especialidad_medica.RulesHint = true;
			treeview_especialidad_medica.RowActivated += on_button_consulta_medica_clicked;  // Doble click selecciono paciente*/
						
			Gtk.TreeViewColumn col_00 = new TreeViewColumn();
			Gtk.CellRendererText cellrt00 = new CellRendererText();
			col_00.Title = "Especialidad";
			col_00.PackStart(cellrt00, true);
			col_00.AddAttribute (cellrt00, "text", 0);
			...
			col_01.Title = "ID Espec.";
			col_01.PackStart(cellrt01, true);
			col_01.AddAttribute (cellrt01, "text", 1);
			//col_00.SortColumnId = (int) Column_notas.col_00;
			
			treeview_especialidad_medica.AppendColumn(col_00);
			treeview_especialidad_medica.AppendColumn(col_01);
		}
	
		void llenado_treeview_espmedicas()
		{
			treeViewEngineEspeMed.Clear();
			NpgsqlConnection conexion; 
			conexion = new NpgsqlConnection (connectionString+nombrebd);
			// Verifica que la base de datos este conectada
			try{
				...
				NpgsqlCommand comando;
				comando = conexion.CreateCommand ();
				comando.CommandText = "SELECT descripcion_especialidad,id_especialidad " +
					"FROM osiris_his_tipo_especialidad " +
					"WHERE notaevolucion_consulta = 'true' " +
					"AND espcialidad_activo = 'true' "+
					"AND id_especialidad = '" +entry_id_especialidad.Text.Trim()+"';";
					
				//Console.WriteLine(comando.CommandText);
				NpgsqlDataReader lector = comando.ExecuteReader ();
				while (lector.Read()){					
					treeViewEngineEspeMed.AppendValues(lector["descripcion_especialidad"].ToString().Trim(),
					                                   int.Parse(lector["id_especialidad"].ToString().Trim()),
					                                   int.Parse(idtipointernamiento.ToString().Trim()),
					                                   2);
				}
			}catch (NpgsqlException ex){
	   			MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
