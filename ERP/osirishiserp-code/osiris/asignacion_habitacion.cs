        ...
        void on_lista_clicked(object sender, EventArgs args)
		{
			...
			if (miResultado == ResponseType.Yes){			
				...
				parametros = new string[,] {
					...
					{ "folio_de_servicio = '","0'," },
					{ "pid_paciente = '","0' " },
					{ "WHERE id_habitacion = '",id_habitacion_proveniente.ToString ()+"' "}
				};
				paraobj = new object[] { entry_folio};
				new osiris.update_registro ("osiris_his_habitaciones", parametros, paraobj);
				//
				parametros = new string[,] {
					{ "id_habitacion = '","1' " },
					{ "WHERE folio_de_servicio = '",entry_folio.Text.ToString ().Trim()+"' "}
				};
				paraobj = new object[] { entry_folio};
				new osiris.update_registro ("osiris_erp_cobros_enca", parametros, paraobj);
				//
				parametros = new string[,] {
					{ "id_quien_asigna,", "'" + LoginEmpleado + "'," }, 
					{ "folio_de_servicio,", "'" + entry_folio.Text.Trim () + "'," },
					{ "pid_paciente,", "'" + pid_pasingado + "'," },
					{ "fecha_de_asignacion,", "'" + query_fecha_de_ocupacion + "'," },
					{ "traspaso,", "'true'," },
					{ "id_habitacion,", "'0'," },
					{ "id_habitacion_anterior,", "'" + id_habitacion_proveniente + "'," },
					{ "dias_de_ocupacion,", "'1' " }
				};
					paraobj = new object[] { entry_folio};
				new osiris.insert_registro ("osiris_his_historial_habitaciones", parametros, paraobj);
				limpia_variables();			
			}else{			   
