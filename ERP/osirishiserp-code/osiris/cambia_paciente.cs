...
//					Daniel Olivares - arcangeldoc@openmailbox.org (pre-Programacion)
//				  (Programacion Mono)
// 				  
// Licencia		: GLP
...
namespace osiris
{
		...
		public cambia_paciente (string LoginEmpleado_,string NomEmpleado_,string AppEmpleado_,string ApmEmpleado_,string nombrebd_,int pidpaciente_)
		{
			...
			if(pidpaciente_ != 0){
				llena_inf_de_paciente(pidpaciente_);
			}
		}	
		...
		{	
			...
			if (this.lista_de_Pacientes.Selection.GetSelected(out model, out iterSelected)) {
				...
				activa_los_entry(true);
				llena_inf_de_paciente(PidPaciente);
		...
		void llena_inf_de_paciente(int pidpaciente_)
		{
			NpgsqlConnection conexion; 
			conexion = new NpgsqlConnection (connectionString+nombrebd );
            // Verifica que la base de datos este conectada
			try{
				...
				comando = conexion.CreateCommand ();
               	comando.CommandText ="SELECT to_char(pid_paciente,'9999999999') AS pidpaciente, "+ 
									...
									"id_empresa,"+
									"municipio_paciente,"+
				                      "activo,religion_paciente,alegias_paciente,lugar_nacimiento_paciente,"+
									"observaciones "+
									"FROM osiris_his_paciente "+
									"WHERE osiris_his_paciente.activo = true "+
		...
		void desactiva_entrys(bool valor)
		{
			...
			entry_lugar_nacimiento.Sensitive = false;
		}
		...
		void on_button_guardar_clicked(object sender, EventArgs args)
		{
			...
			try
				...
				comando.CommandText = "UPDATE osiris_his_paciente "+
									...
									"religion_paciente = '"+entry_religion.Text.ToUpper().Trim()+"',"+
									"alegias_paciente = '"+entry_alegias.Text.ToUpper().Trim()+"',"+
				                    "lugar_nacimiento_paciente = '" + entry_lugar_nacimiento.Text.ToUpper().Trim()+"',"+			
