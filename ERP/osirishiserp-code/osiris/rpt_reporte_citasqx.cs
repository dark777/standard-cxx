...
namespace osiris
{
	public class rpt_reporte_citasqx
	{
		...
		public rpt_reporte_citasqx (object _treeview_lista_agenda_, object _treeViewEngineListaCitas_,string fechainicio_, string fechatermino_)
		{
		...
		void ejecutar_consulta_reporte(PrintContext context)
		{
			...
			if (treeViewEngineListaCitas.GetIterFirst (out iter)){
				...
				cr.MoveTo(05*escala_en_linux_windows,comienzo_linea*escala_en_linux_windows);			layout.SetText("Observaciones :"+(string) treeview_lista_agenda.Model.GetValue (iter,14));			Pango.CairoHelper.ShowLayout (cr, layout);
				salto_de_pagina(cr,layout);
				comienzo_linea += separacion_linea;
				...
				salto_de_pagina(cr,layout);
				comienzo_linea += separacion_linea;
				cr.MoveTo(750*escala_en_linux_windows, comienzo_linea*escala_en_linux_windows);
		...
		void imprime_encabezado(Cairo.Context cr,Pango.Layout layout)
		{
			...
			fontSize = 10.0;
			desc.Size = (int)(fontSize * pangoScale);					layout.FontDescription = desc;
			cr.MoveTo(210*escala_en_linux_windows, 25*escala_en_linux_windows);			layout.SetText("CALENDARIO DE CITAS Y CX. PROGRAMADAS DEL "+fechainicio+"  HASTA "+fechatermino);					Pango.CairoHelper.ShowLayout (cr, layout);
			// Creando el Cuadro de Titulos
