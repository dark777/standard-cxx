...
//				 Ing. Daniel Olivares - arcangeldoc@openmailbox.org (Ajustes y Pre-programacion)	
// 				  
// Licencia		: GLP
...
namespace osiris
{
	public class costeo_productos
	{
		...
		public costeo_productos(string LoginEmp_, string NomEmpleado_, string AppEmpleado_, string ApmEmpleado_, string nombrebd_) 
		{
			...
			button_selecciona.Clicked += new EventHandler(on_selec_folio_clicked);
			entry_folio_servicio.KeyPressEvent += onKeyPressEvent_enter_folio;

			button_salir.Clicked += new EventHandler(on_cierraventanas_clicked);
			button_imprimir.Clicked += new EventHandler(on_imprime_clicked);
			button_actualiza.Clicked += new EventHandler(on_button_exportar_clicked);
		}
			...
			if ((string) this.entry_folio_servicio.Text != ""){
				...
				try{
					...
					if(lector.Read()){
						...
						treeViewEngineProducto.AppendValues(total_cantaplicada.ToString("F"),
					...
					entry_porcentage_total.Text = porcentage_total.ToString("C");
					//this.entry_total_esperado.Text = esperado.ToString("C");
					entry_total_comprado.Text = comprado.ToString("C");
					//this.entry_dif_cobrado_esperado.Text = diferencia.ToString("C");
					entry_total_cobrado.Text = cobrado.ToString("C");
					entry_dif_compra_cobro.Text = diferencia2.ToString("C");
