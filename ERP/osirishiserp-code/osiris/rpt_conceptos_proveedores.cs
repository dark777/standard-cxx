...
namespace osiris
{
	public class rpt_conceptos_proveedores
	{
		...
		[Widget] Gtk.CheckButton checkbutton_costeo_solicitud = null;
		...
		public rpt_conceptos_proveedores ()
		{
			...
			llenadogrupoprod();
			llenado_combobox(0,"",combobox_sub_almacenes,"sql","SELECT * FROM osiris_almacenes WHERE activo = 'true' ORDER BY descripcion_almacen;","descripcion_almacen","id_almacen",args_args,args_id_array,"");
		...
		void on_button_exportar_sheet_clicked(object sender, EventArgs args)
		{
			...
			if ((bool)checkbutton_herr_adicionales.Active == true){
				...
				switch(tiporeporte_herramientas){
					case 1:
						query_consulta = "SELECT osiris_erp_requisicion_deta.id_producto AS idproducto_osiris,descripcion_producto," +
							...
							"AVG(osiris_erp_requisicion_deta.precio_costo_prov_selec) AS avg," +
							"MAX(osiris_erp_requisicion_deta.precio_costo_prov_selec) AS max," +
							"MIN(osiris_erp_requisicion_deta.precio_costo_prov_selec) AS min," +
					...
					case 2:
						...
						query_consulta = "SELECT to_char(fechahora_creacion,'yyyy-MM-dd') AS fechacaptura,to_char(osiris_erp_factura_compra_enca.fecha_factura,'yyyy-MM-dd') AS fechafactura," +
							...
							"osiris_erp_emisor.emisor AS receptorfactura,uuid " +
						...
						string[] args_names_field = {"fechacaptura","fechaentrada","fechafactura","numerofactura","descripcion_proveedor","ordencompra","subtotal_factura","iva_factura","total_factura","idreceptor","receptorfactura","uuid"};
						string[] args_type_field = {"string","string","string","string","string","string","float","float","float","float","float","string","string"};
						string[] args_field_text = {""};
						string[] args_more_title = {""};
						string[,] args_formulas = {{"",""}};
						string[,] args_width = {{"4","9cm"},{"11","7cm"}};
						new osiris.class_traslate_spreadsheet(query_consulta,args_names_field,args_type_field,false,args_field_text,"",false,args_more_title,args_formulas,args_width);
					break;
				}
			}else{
				verifica_checkbutton_prov();
				...
				query_consulta = "SELECT to_char(osiris_erp_factura_compra_enca.fecha_factura,'yyyy-MM-dd') AS fechafactura,osiris_erp_factura_compra_enca.numero_factura_proveedor AS numerofactura," +
					...
					"osiris_erp_requisicion_deta.id_requisicion AS numero_requisicion,osiris_erp_requisicion_deta.numero_orden_compra,osiris_erp_requisicion_deta.uuid AS folio_fiscal," +
					"osiris_productos.id_grupo_producto,descripcion_grupo_producto,descripcion_grupo1_producto,descripcion_grupo2_producto "+
					"FROM osiris_erp_factura_compra_enca,osiris_erp_proveedores,osiris_erp_requisicion_deta,osiris_productos,osiris_grupo_producto,osiris_grupo1_producto,osiris_grupo2_producto " +
					"WHERE osiris_erp_factura_compra_enca.id_proveedor = osiris_erp_proveedores.id_proveedor " +
					"AND osiris_erp_factura_compra_enca.id_proveedor = osiris_erp_requisicion_deta.id_proveedor " +
					...
					query_in_grupo+
					"ORDER BY to_char(osiris_erp_factura_compra_enca.fecha_factura,'yyyy-MM-dd'),osiris_erp_factura_compra_enca.id_proveedor,numerofactura;";
				string[] args_names_field = {"fechafactura","numerofactura","descripcion_proveedor","idproducto_osiris","descrip_prod_osiris","cantidad_comprada","costo_producto_compra","cantidad_de_embalaje","costoxunidad_compra","cantidad_recibida","costo_producto_osiris","cantidad_de_embalaje_osiris","costo_unitario_osiris","id_producto_proveedor","descripcion_producto_proveedor","tipo_unidad_producto","lote_producto","caducidad_producto","numero_requisicion","numero_orden_compra","folio_fiscal","descripcion_grupo_producto","descripcion_grupo1_producto","descripcion_grupo2_producto"};
				string[] args_type_field = {"string","string","string","string","string","float","float","float","float","float","float","float","float","string","string","string","string","string","float","float","string","string","string","string"};
				string[] args_field_text = {""};
				string[] args_more_title = {""};
				string[,] args_formulas = {{"",""}};
				string[,] args_width = {{"",""}};
               	...
               	if(lector.Read()){
					...
					while(lector.Read()){
						if(codigoproducto != lector["idproducto_osiris"].ToString().Trim()){
							...
							cr.MoveTo(05*escala_en_linux_windows,comienzo_linea*escala_en_linux_windows);			layout.SetText(lector["idproducto_osiris"].ToString().Trim());		Pango.CairoHelper.ShowLayout (cr, layout);
							if(lector["descripcion_producto"].ToString().Length > 60){
								cr.MoveTo(55*escala_en_linux_windows, comienzo_linea*escala_en_linux_windows);			layout.SetText(lector["descripcion_producto"].ToString().Trim().Substring(0,60));				Pango.CairoHelper.ShowLayout (cr, layout);				
							}else{
								cr.MoveTo(55*escala_en_linux_windows, comienzo_linea*escala_en_linux_windows);			layout.SetText(lector["descripcion_producto"].ToString().Trim());				Pango.CairoHelper.ShowLayout (cr, layout);				
		...
		void verifica_checkbutton_fechas()
		{
			...
			if(checkbutton_todas_fechas.Active == true){
				...
				query_rango_fechas2 ="AND to_char(osiris_erp_factura_compra_enca.fecha_factura,'yyyy') = '"+(string) entry_ano_inicial.Text.ToString()+"' "+
									  "AND to_char(osiris_erp_factura_compra_enca.fecha_factura,'MM') >= '"+(string) entry_mes_inicial.Text.ToString()+"' "+
									 "AND to_char(osiris_erp_factura_compra_enca.fecha_factura,'MM') <= '"+(string) entry_mes_final.Text.ToString()+"' ";
				active_checkbutton = true;
			}			
