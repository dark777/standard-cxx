...
namespace osiris
{
	public class movimientos_productos
	{
		...
		public movimientos_productos (string LoginEmp_,string NomEmpleado_,string AppEmpleado_,string ApmEmpleado_,string nombrebd_,string tipo_reporte_,string query_sql_)
		{
			...
			if(tipo_reporte_ == "envios_subalmacenes"){
				crea_treeview_selec("envios_subalmacenes");
				...
				button_imprimir_movimiento.Clicked += new EventHandler(on_button_imprimir_envsubalma_clicked);
			}
			...
			button_crealist_prod.Sensitive = false;
		...
		void on_checkbutton_grupos_clicked(object sender, EventArgs args)
		{
			...
			if(checkbutton_recive.Name == "checkbutton_grupo"){
				button_crealist_prod.Sensitive = (bool) checkbutton_grupo.Active;
		...
		void on_checkbutton_todos_departamentos_clicked(object sender, EventArgs args)
		{
			if (checkbutton_todos_departamentos.Active == true){
				...
				if(tipo_reporte == "envios_subalmacenes"){
					query_departamento = "AND osiris_his_solicitudes_deta.id_almacen = '"+idtipoadmisiones.ToString()+"' ";
				}
				if(tipo_reporte == "cargos_pacientes"){
					query_departamento = "AND osiris_erp_cobros_deta.id_tipo_admisiones = '"+idtipoadmisiones.ToString()+"' ";
				}
				if(tipo_reporte == "productos_requisados"){
					query_departamento = "";
				}
				if(tipo_reporte == "cargos_x_fecha"){
					query_departamento = " AND osiris_grupo_producto.id_grupo_producto = '"+idtipoadmisiones.ToString()+"' ";
				}
			}
		}
		...
		{ 	
			button_busca_producto.Sensitive = !(bool) checkbutton_todos_productos.Active;
			entry_descrip_producto.Sensitive = !(bool) checkbutton_todos_productos.Active;
			button_crealist_prod.Sensitive = (bool) checkbutton_todos_productos.Active;
			combobox_grupo.Sensitive = (bool) checkbutton_todos_productos.Active;
			checkbutton_grupo1.Sensitive = (bool) checkbutton_todos_productos.Active;
			combobox_grupo1.Sensitive = (bool) checkbutton_todos_productos.Active;
			checkbutton_grupo2.Sensitive = (bool) checkbutton_todos_productos.Active;
			combobox_grupo2.Sensitive = (bool) checkbutton_todos_productos.Active;
		}
		...
		void onComboBoxChanged_llenado (object sender, EventArgs args)
		{
			...
			if (onComboBoxChanged.GetActiveIter (out iter)){
				...
				case "combobox_departamentos":
					idtipoadmisiones = (int) onComboBoxChanged.Model.GetValue(iter,1);
					if(tipo_reporte == "envios_subalmacenes"){
						query_departamento = " AND osiris_his_solicitudes_deta.id_almacen = '"+Convert.ToString((int) onComboBoxChanged.Model.GetValue(iter,1)).ToString()+"' ";	    			    	
					}
					if(tipo_reporte == "cargos_pacientes"){
						query_departamento = " AND osiris_erp_cobros_deta.id_tipo_admisiones = '"+Convert.ToString((int) onComboBoxChanged.Model.GetValue(iter,1)).ToString()+"' ";
					}
					if(tipo_reporte == "productos_requisados"){
						query_departamento = "";
					}					
					if (tipo_reporte == "cargos_x_fecha"){
						query_departamento = " AND osiris_grupo_producto.id_grupo_producto = '"+Convert.ToString((int) onComboBoxChanged.Model.GetValue(iter,1)).ToString()+"' ";
					}
					break;
				case "combobox_grupo":
		...
		void on_button_imprimir_movimiento_clicked(object sender, EventArgs args)
		{
			...
			if (this.treeViewEngineResumen.GetIterFirst (out iter)){
				if(checkbutton_export_ods.Active == true){
					string query_sql = query_consulta;
					string[] args_names_field = {"fechahoracreacion","cantidadaplicada","idproducto","descripcion_producto","precio_producto","precio_costo_unitario","foliodeservicio","pidpaciente","nombre_paciente","cliente","descripcion_tipo_paciente","descripcion_grupo_producto","descripcion_grupo1_producto","descripcion_grupo2_producto"};
					string[] args_type_field = {"string","float","string","string","float","float","float","float","string","string","string","string","string","string"};
					string[] args_field_text = {""};
					string[] args_more_title = {""};
					string[,] args_formulas = {{"",""}};
					string[,] args_width = {{"3","7cm"},{"8","8cm"},{"9","8cm"},{"10","8cm"},{"11","6cm"},{"12","6cm"},{"13","6cm"}};
		...
		void on_button_imprimir_envsubalma_clicked(object sender, EventArgs args)
		{
			TreeIter iter;
			if (this.treeViewEngineResumen.GetIterFirst (out iter)){
				if(checkbutton_export_ods.Active == true){
					string query_sql = query_consulta;
					string[] args_names_field = {"descripcion_almacen","folio_de_solicitud","idproducto","descripcion_producto","fechahorasolic","fechahoraautoriz","tiemposurtido","cantidad_solicitada","cantidad_autorizada","tipo_solicitud","foliodeatencion","pidpaciente","nombre_completo","descripcion_grupo_producto","descripcion_grupo1_producto","descripcion_grupo2_producto"};
					string[] args_type_field = {"string","float","string","string","string","string","string","float","float","string","float","float","string","string","string","string"};
					string[] args_field_text = {""};
					string[] args_more_title = {""};
					string[,] args_formulas = {{"",""}};
					string[,] args_width = {{"",""}};
		...
		void crea_treeview_selec(string nombre_reporte)
		{
			...
			if(nombre_reporte == "envios_subalmacenes" || nombre_reporte == "cargos_pacientes" || nombre_reporte == "productos_requisados" || nombre_reporte == "productos_comprados"){
		...
		void llenando_productos()
		{
			...
			if(tipo_reporte == "productos_requisados"){
				query_fechas = "AND to_char(osiris_erp_requisicion_deta.fechahora_requisado,'yyyy-MM-dd') >= '"+entry_ano1.Text.Trim()+"-"+entry_mes1.Text+"-"+entry_dia1.Text+"' "+
						"AND to_char(osiris_erp_requisicion_deta.fechahora_requisado,'yyyy-MM-dd') <= '"+entry_ano2.Text.Trim()+"-"+entry_mes2.Text+"-"+entry_dia2.Text+"' ";
				campo_filtrado = "AND osiris_erp_requisicion_deta.id_producto IN('";
				filtro_producto = true;
			}
			if(tipo_reporte == "cargos_x_fecha"){
				query_fechas = "AND to_char(osiris_erp_cobros_deta.fechahora_creacion,'yyyy-MM-dd') >= '"+entry_ano1.Text.Trim()+"-"+entry_mes1.Text+"-"+entry_dia1.Text+"' "+
			...
			if(filtro_producto == true){
				...
				if (this.checkbutton_todos_productos.Active == false ){
					if (this.treeViewEngineSelec.GetIterFirst (out iter)){
						if (idtipoadmisiones != 0 || this.checkbutton_todos_departamentos.Active == true){ 
		...
		{	
			...
			if(tipo_reporte == "envios_subalmacenes"){
				query_consulta =  "SELECT to_char(osiris_his_solicitudes_deta.id_producto,'999999999999') AS idproducto,descripcion_almacen," +
				                 "descripcion_producto,folio_de_solicitud,tipo_solicitud,envio_directo," +
				                 "osiris_his_solicitudes_deta.fechahora_autorizado - osiris_his_solicitudes_deta.fechahora_solicitud AS tiemposurtido," +
				        ...
				        "osiris_his_solicitudes_deta.id_almacen,descripcion_grupo_producto,descripcion_grupo1_producto,descripcion_grupo2_producto "+
				        "FROM osiris_his_solicitudes_deta,osiris_productos,osiris_almacenes,osiris_his_paciente,osiris_grupo_producto,osiris_grupo1_producto,osiris_grupo2_producto " +
			...
			if(tipo_reporte == "cargos_pacientes"){
				query_consulta = "SELECT to_char(SUM(osiris_erp_cobros_deta.cantidad_aplicada),'999999999.99') AS cantidadaplicada,to_char(osiris_erp_cobros_deta.id_producto,'999999999999') AS idproducto,descripcion_producto," +
						"osiris_erp_cobros_deta.precio_producto,osiris_erp_cobros_deta.precio_costo_unitario,"+
						"to_char(osiris_erp_cobros_deta.folio_de_servicio,'9999999999') AS foliodeservicio,to_char(osiris_erp_cobros_deta.fechahora_creacion,'yyyy-MM-dd HH24:mi') AS fechahoracreacion,"+
						...
						"to_char(osiris_erp_cobros_deta.id_tipo_admisiones,'9999999999') AS idtipoadmisiones,descripcion_admisiones,descripcion_empresa,descripcion_aseguradora," +
					"descripcion_empresa || ' ' || descripcion_aseguradora AS cliente," +
					...
					"FROM osiris_erp_cobros_deta,osiris_productos,osiris_his_paciente,osiris_his_tipo_admisiones,osiris_erp_cobros_enca,osiris_empresas,osiris_aseguradoras,osiris_grupo_producto,osiris_grupo1_producto,osiris_grupo2_producto,osiris_his_tipo_pacientes "+
					...
					"AND osiris_erp_cobros_enca.id_tipo_paciente = osiris_his_tipo_pacientes.id_tipo_paciente "+
						...
						query_fechas_+
						query_departamento+
					"GROUP BY osiris_erp_cobros_deta.id_producto,descripcion_producto,osiris_erp_cobros_deta.precio_producto,osiris_erp_cobros_deta.precio_costo_unitario," +
					"osiris_erp_cobros_deta.folio_de_servicio,osiris_erp_cobros_deta.fechahora_creacion,osiris_erp_cobros_deta.pid_paciente," +
					...
					"osiris_erp_cobros_deta.id_tipo_admisiones,descripcion_admisiones,descripcion_tipo_paciente,descripcion_empresa,descripcion_aseguradora,osiris_erp_cobros_enca.id_tipo_paciente,descripcion_grupo_producto,descripcion_grupo1_producto,descripcion_grupo2_producto "+
						"ORDER BY osiris_erp_cobros_deta.id_tipo_admisiones,osiris_erp_cobros_deta.id_producto,"+
						"osiris_his_paciente.nombre1_paciente || ' ' || osiris_his_paciente.nombre2_paciente || ' ' || osiris_his_paciente.apellido_paterno_paciente || ' ' || osiris_his_paciente.apellido_materno_paciente;";
			}
			
			if(tipo_reporte == "productos_requisados"){
				query_consulta = "SELECT to_char(osiris_erp_requisicion_deta.id_producto,'999999999999') AS idproducto,descripcion_producto," +
						"to_char(osiris_erp_requisicion_deta.fechahora_requisado,'yyyy-MM-dd') AS fechahoracreacion,"+
						"to_char(osiris_productos.cantidad_de_embalaje,'999999999.99') AS embalaje,"+
						"to_char(osiris_erp_requisicion_deta.cantidad_solicitada,'999999999.99') AS cantidadsolicitada,to_char(osiris_erp_requisicion_deta.cantidad_comprada,'999999999.99') AS cantidadcomprada," +
						"to_char(id_requisicion,'9999999999') AS idrequisicion,to_char(osiris_erp_requisicion_deta.fecha_deorden_compra,'yyyy-MM-dd') AS fecha_ordencompra," +
						"to_char(osiris_erp_requisicion_deta.numero_orden_compra,'9999999999') AS nroordencompra " +
						"FROM osiris_erp_requisicion_deta,osiris_productos " +
						"WHERE osiris_erp_requisicion_deta.id_producto = osiris_productos.id_producto "+
						productos_seleccionado_+
						query_fechas_+
						" ORDER BY to_char(osiris_erp_requisicion_deta.fechahora_requisado,'yyyy-MM-dd');";	
			}
			...
			try{
				...
				while (lector.Read()){					
					if(tipo_reporte == "envios_subalmacenes"){
						//tipo_solicitud,envio_directo
						tiposolicitud = (string) lector["tipo_solicitud"].ToString().Trim();
						if((bool) lector["envio_directo"] == true){
							tiposolicitud = "ENVIO DIRECTO";
						}
					...
					if(tipo_reporte == "productos_requisados"){
						treeViewEngineResumen.AppendValues((string) lector["idproducto"],
										(string) lector["descripcion_producto"],
						                (string) lector["embalaje"],
						                (string) lector["cantidadsolicitada"],
						                (string) lector["cantidadcomprada"],
						                (string) lector["fechahoracreacion"],
						                (string) lector["idrequisicion"].ToString().Trim(),
						                (string) lector["fecha_ordencompra"],
						                (string) lector["nroordencompra"]);
					}					
					if(tipo_reporte == "cargos_x_fecha"){
		...
		void crea_treeview_envios()
		{
			...
			TreeViewColumn col_tipo_solic = new TreeViewColumn();
			CellRendererText cellr11 = new CellRendererText();
			col_tipo_solic.Title = "Tipo Solic.";
			col_tipo_solic.PackStart(cellr11, true);
			col_tipo_solic.AddAttribute (cellr11, "text", 11);
			col_tipo_solic.SortColumnId = (int) Column_resumen.col_tipo_solic;
			...
			lista_resumen_productos.AppendColumn(col_tipo_solic);
		}
		...
		{        
            ...
            col_nroatencion,
			col_nombre_paciente,
			col_nro_expediente,col_tipo_solic,
		}		
		...
		void crea_treeview_cargos_pacientes()
		{
			...
			CellRendererText cellr4 = new CellRendererText();
			col_pid_paciente.Title = "N° Expediente"; // titulo de la cabecera de la columna, si está visible
		...
		void crea_treeview_prod_requisados()
		{
			treeViewEngineResumen = new TreeStore(
								typeof(string), 
								typeof(string),
								typeof(string),
								typeof(string),
			...
			CellRendererText cellr0 = new CellRendererText();
			col_id_producto.Title = "ID Producto."; // titulo de la cabecera de la columna, si está visible
			col_id_producto.PackStart(cellr0, true);
			col_id_producto.AddAttribute (cellr0, "text", 0);
			//col_id_producto.SortColumnId = (int) Column_resumen1.col_id_producto;
			...
			CellRendererText cellr2 = new CellRendererText();
			col_embalaje.Title = "Pack/Embalaje";
			col_embalaje.PackStart(cellr2, true);
			col_embalaje.AddAttribute (cellr2, "text", 2);
			//col_fechsolicitada.SortColumnId = (int) Column_resumen.col_fechsolicitada;			
			
			TreeViewColumn col_cantrequi = new TreeViewColumn();
			CellRendererText cellr3 = new CellRendererText();
			col_cantrequi.Title = "Cant.Requi.";
			col_cantrequi.PackStart(cellr3, true);
			col_cantrequi.AddAttribute (cellr3, "text", 3);
			//col_fechsolicitada.SortColumnId = (int) Column_resumen.col_fechsolicitada;
			
			TreeViewColumn col_cantcompra = new TreeViewColumn();
			CellRendererText cellr4 = new CellRendererText();
			col_cantcompra.Title = "Cant.Compra";
			col_cantcompra.PackStart(cellr4, true);
			col_cantcompra.AddAttribute (cellr4, "text", 4);
			//col_fechsolicitada.SortColumnId = (int) Column_resumen.col_fechsolicitada;
			
			TreeViewColumn col_fechrequi = new TreeViewColumn();
			CellRendererText cellr5 = new CellRendererText();
			col_fechrequi.Title = "Fech.Requi.";
			col_fechrequi.PackStart(cellr5, true);
			col_fechrequi.AddAttribute (cellr5, "text", 5);
			//col_fechsolicitada.SortColumnId = (int) Column_resumen.col_fechsolicitada;
			
			TreeViewColumn col_nrorequi = new TreeViewColumn();
			CellRendererText cellr6 = new CellRendererText();
			col_nrorequi.Title = "N° Requi.";
			col_nrorequi.PackStart(cellr6, true);
			col_nrorequi.AddAttribute (cellr6, "text", 6);
			//col_nrosolicitud.SortColumnId = (int) Column_resumen.col_nrosolicitud;
			
			TreeViewColumn col_fechoc = new TreeViewColumn();
			CellRendererText cellr7 = new CellRendererText();
			col_fechoc.Title = "Fech.OC.";
			col_fechoc.PackStart(cellr7, true);
			col_fechoc.AddAttribute (cellr7, "text", 7);
			//col_fechsolicitada.SortColumnId = (int) Column_resumen.col_fechsolicitada;
			
			TreeViewColumn col_nrooc = new TreeViewColumn();
			CellRendererText cellr8 = new CellRendererText();
			col_nrooc.Title = "N° OC.";
			col_nrooc.PackStart(cellr8, true);
			col_nrooc.AddAttribute (cellr8, "text", 8);
			//col_nrosolicitud.SortColumnId = (int) Column_resumen.col_nrosolicitud;
			
			TreeViewColumn col_nomprove = new TreeViewColumn();
			CellRendererText cellr9 = new CellRendererText();
			col_nomprove.Title = "Proveedor";
			col_nomprove.PackStart(cellr9, true);
			col_nomprove.AddAttribute (cellr9, "text", 9);
			col_nomprove.Resizable = true;
			cellr9.Width = 300;
			//col_nrosolicitud.SortColumnId = (int) Column_resumen.col_nrosolicitud;
			
			TreeViewColumn col_nrofactura = new TreeViewColumn();
			CellRendererText cellr10 = new CellRendererText();
			col_nrofactura.Title = "N° Factura";
			col_nrofactura.PackStart(cellr10, true);
			col_nrofactura.AddAttribute (cellr10, "text",10);
			col_nrofactura.Resizable = true;
			cellr10.Width = 100;
			//col_nrosolicitud.SortColumnId = (int) Column_resumen.col_nrosolicitud;
			
			lista_resumen_productos.AppendColumn(col_id_producto);
			lista_resumen_productos.AppendColumn(col_descripcion);
			lista_resumen_productos.AppendColumn(col_embalaje);
			lista_resumen_productos.AppendColumn(col_cantrequi);
			lista_resumen_productos.AppendColumn(col_cantcompra);
			lista_resumen_productos.AppendColumn(col_fechrequi);
			lista_resumen_productos.AppendColumn(col_nrorequi);
			lista_resumen_productos.AppendColumn(col_fechoc);
			lista_resumen_productos.AppendColumn(col_nrooc);
			lista_resumen_productos.AppendColumn(col_nomprove);
			lista_resumen_productos.AppendColumn(col_nrofactura);
		}
		...
		////////// llena la lista de productos//////////////////////////////
	 	...
	 	void llenando_lista_de_productos()
	 	{
			...
			if(radiobutton_nombre.Active == true) {
				query_tipo_busqueda = "AND osiris_productos.descripcion_producto LIKE '%"+entry_expresion.Text.ToUpper().Trim()+"%' ";
			}			
			if(radiobutton_codigo.Active == true){
				query_tipo_busqueda = "AND osiris_productos.id_producto LIKE '%"+entry_expresion.Text.Trim()+"%' ";
			}
			NpgsqlConnection conexion; 
			conexion = new NpgsqlConnection (connectionString+nombrebd);
			//Verifica que la base de datos este conectada
		...
		void on_button_crealist_prod_clicked(object sender, EventArgs args)
		{
			...
			if((bool) checkbutton_grupo.Active == true){
				...
				llenado_de_productos_grupos();
			}
		}
		
		void llenado_de_productos_grupos()
		{
			//treeViewEngineSelec.Clear();
			NpgsqlConnection conexion; 
			conexion = new NpgsqlConnection (connectionString+nombrebd);
			...
			try{
				...
				while (lector.Read()){
					...
					preciomasiva = tomaprecio + calculodeiva;
					treeViewEngineSelec.AppendValues(
									(string) lector["codProducto"] ,
									(string) lector["descripcion_producto"]);
