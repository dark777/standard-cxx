...
//				: Ing. Juan Antonio Peña (Adecuaciones y Colaboracion) 
// 				  
// Licencia		: GLP
//////////////////////////////////////////////////////////
//
...
namespace osiris
{
	public class caja_cobro
	{
		...
		[Widget] Gtk.TreeView treeview_atenciones;
		...
		[Widget] Gtk.Button button_modifica_fechdeta = null;
		...
		[Widget] Gtk.Button button_quitar_altapx = null;
		...
		[Widget] Gtk.Button button_solicitud_almacen = null;
		[Widget] Gtk.Button button_solicitud_consulta = null;
		...
		[Widget] Gtk.Button button_lista_precios = null;
		
		Gtk.Entry entry_id_doctor = null;
		...
		[Widget] Gtk.Label label433 = null;
		...
		[Widget] Gtk.ComboBox combobox_tipo_honorario = null;
		[Widget] Gtk.Entry entry_motivohonorario = null;
		[Widget] Gtk.Entry entry_motivo_ingreso = null;
		[Widget] Gtk.Entry entry_idmedico = null;
		[Widget] Gtk.Entry entry_dia_visitamed = null;
		[Widget] Gtk.Entry entry_mes_visitamed = null;
		[Widget] Gtk.Entry entry_ano_visitamed = null;
		[Widget] Gtk.Button button_agregar_honorario = null;
		...
		//Ventana para editar el concepto
		[Widget] Gtk.Window edita_concepto = null;
		[Widget] Gtk.Entry entry_folio_servicio_edit = null;
		[Widget] Gtk.Entry entry_pid_paciente_edit = null;
		[Widget] Gtk.Entry entry_nombre_paciente_edit = null;
		[Widget] Gtk.Entry entry_descrip_concepto = null;
		...
		[Widget] Gtk.Entry entry_monto_actual = null;
		[Widget] Gtk.Entry entry_monto_edit = null;
		[Widget] Gtk.Entry entry_numero_regedit = null;
		[Widget] Gtk.Entry entry_cantapli_regedit = null;
		[Widget] Gtk.Entry entry_valoriva_regedit = null;
		[Widget] Gtk.Button button_graba_editconcep = null;
		[Widget] Gtk.Statusbar statusbar_edita_concepto = null;
		...
		private TreeStore treeViewEngine_atenciones;
		...
		int idtipohonorario = 0;				// Toma el tipo del honorario medico
		...
		bool aplica_precios_tipopx = false;			// toma la lista de precio del tipo de paciente
		bool aplica_precios_sub_tipopx = false;
		
		bool aplica_iva_servmedico = false;
				
		int idtipointernamiento = 10; 	 		// Toma el valor del tipo de internamiento
		string descripinternamiento = "CAJA";  		// Toma la descripcion del internamiento
		...
		int idmedico_ingreso;		// medico de primer ingreso
		int idmedico_tratante;		// medico quient trata al paciente
		...
		float totalivacosto = 0;
		float subtotalcon_impuesto_costo = 0;
		float subtotalsin_impuesto_costo = 0;
				
		// Variables publicas para le rango de fecha procedimiento
		string fecha_rango_1 = DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss");
		string fecha_rango_2 = DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss");
		
		bool aplico_cargos = false;
		bool altamedicapaciente = false;
		bool tienepagare = false;
		...
		// ANAHUAC
		// 642
		string expediente_ventapublico = "17249";

		string[] args_args = {""};
		string[] args_tipohonormedico = {"","INCLUIDO EN LA CUENTA","AGREGAR A LA CUENTA"};
		...
		public caja_cobro(string LoginEmp, string NomEmpleado_, string AppEmpleado_, string ApmEmpleado_, string nombrebd_, int idsubalmacen_, int idcentro_costo_) 
		{
			...
			button_quitar_altapx.Clicked += new EventHandler(on_button_quitar_altapx_clicked);
			...
			button_solicitud_almacen.Clicked += new EventHandler(on_solicitud_labrx_clicked);
			button_solicitud_consulta.Clicked += new EventHandler(on_button_solicitud_consulta_clicked);
			button_lista_precios.Clicked += new EventHandler(on_button_lista_precios_clicked);
			button_modifica_fechdeta.Clicked += new EventHandler(on_button_modifica_fechdeta_clicked);
			...
			button_solicitud_lab.Sensitive = false;
			button_solicitud_rx.Sensitive = false;
			button_solicitud_almacen.Sensitive = false;
			button_solicitud_vision.Sensitive = false;
			button_quitar_altapx.Sensitive = false;
			button_modifica_fechdeta.Sensitive = false;
			...
			entry_folio_servicio.ModifyBase(StateType.Normal,new Gdk.Color(17,205,41));
		}
		...
		void onComboBoxChanged_llenado (object sender, EventArgs args)
		{
			...
			if (onComboBoxChanged.GetActiveIter (out iter)){
				...
				case "combobox_tipo_honorario":
					idtipohonorario = (int) onComboBoxChanged.Model.GetValue(iter,1);
					break;
				case "combobox_tipocomprobante":
		...
		{
			if((string) classpublic.lee_registro_de_tabla("osiris_empleado","exportar_proc_cobranza","WHERE exportar_proc_cobranza = 'true' AND login_empleado = '"+LoginEmpleado+"' ","exportar_proc_cobranza","bool") == "True"){
				string query_sql = "SELECT osiris_erp_cobros_deta.folio_de_servicio AS foliodeservicio,descripcion_producto,to_char(osiris_erp_cobros_deta.id_producto,'999999999999') AS idproducto, "+
					...
					"to_char(osiris_erp_cobros_deta.precio_producto,'99999999.99') AS preciounitario,"+
						...
						"osiris_erp_cobros_deta.precio_costo_unitario,to_char(osiris_erp_cobros_deta.iva_producto,'9999999.99') AS montoiva,"+
						"to_char(osiris_erp_cobros_deta.cantidad_aplicada * osiris_erp_cobros_deta.precio_costo_unitario,'99999999.99') AS ppcantidad_costo," +
				...
				string[] args_names_field = {"foliodeservicio","descripcion_producto","idproducto","cantidadaplicada","preciounitario","ppcantidad","montoiva","precio_costo_unitario","ppcantidad_costo","fechcreacion","descripcion_admisiones","descripcion_grupo_producto"};
				string[] args_type_field = {"float","string","string","float","float","float","float","float","float","string","string","string"};
		...
		void on_button_venta_publico_clicked(object sender, EventArgs args)
		{
			...
			// ANAHUAC
			// 802			
			new osiris.registro_paciente_busca("selecciona",LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,expediente_ventapublico);
		}
		
		void on_button_modifica_fechdeta_clicked(object sender, EventArgs args)
		{
			if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_cambiafecha","WHERE acceso_cambiafecha = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_cambiafecha","bool") == "True"){
				new cambia_fecha_cargo(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,PidPaciente,folioservicio,entry_nombre_paciente.Text.Trim());
			}else{
				MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
									MessageType.Info,ButtonsType.Ok,"No tiene Permiso para esta Opcion");
				msgBox.Run ();msgBox.Destroy();
		...
		void on_button_traspasa_productos_clicked(object sender, EventArgs args)
		{
			...
			if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_traspaso_ctas","WHERE acceso_traspaso_ctas = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_traspaso_ctas","bool") == "True"){
				TreeIter iter;
 				TreeModel model;
			...
			if (miResultado == ResponseType.Yes){
				...
				try{
					...
					if(lector.Read()){
						...
						llenado_de_productos_aplicados( (string) entry_folio_servicio.Text,true );
		...
		void onComboBoxChanged_tipo_admision (object sender, EventArgs args)
		{
			...
			if (sender == null){
	  		...
	  		if (combobox_tipo_admision.GetActiveIter (out iter)){
		    		...
		    		descripinternamiento = combobox_tipo_admision.Model.GetValue(iter,0).ToString().ToUpper();
	     	}
		}
		...
		{
			if((bool) altamedicapaciente == false){
		...
		void on_button_quitar_altapx_clicked(object sender, EventArgs args)
		{
			if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_quitar_altamed","WHERE acceso_quitar_altamed = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_quitar_altamed","bool") == "True"){
					...
					try{
						...
						comando = conexion.CreateCommand ();
 						comando.CommandText = "UPDATE osiris_erp_cobros_enca "+
									"SET alta_paciente = 'false', "+
									"id_habitacion = '1',"+
									"id_empleado_alta_paciente = '',"+
									"historial_altamedica = historial_altamedica || '"+"false;"+LoginEmpleado+";"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"\n' "+
				...
				try{
					...
					comando = conexion.CreateCommand ();
 					comando.CommandText = "UPDATE osiris_erp_cobros_enca "+
									...
									"historial_altamedica = historial_altamedica || '"+"true;"+LoginEmpleado+";"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"\n' "+
									"WHERE  folio_de_servicio =  '"+this.folioservicio+"';";
					//Console.WriteLine(comando.CommandText);
					comando.ExecuteNonQuery();
    	    	    comando.Dispose();
    	    	    conexion.Close();
    	    	    altamedicapaciente = true;
					NpgsqlConnection conexion1; 
					conexion1 = new NpgsqlConnection (connectionString+nombrebd);
	    	        // Verifica que la base de datos este conectada
					try{
						...
						button_quitar_altapx.Sensitive = true;
		...
		void cierre_de_procedimiento()
		{
			...
			if(miResultado == ResponseType.Yes){
				...
				llenado_de_material_aplicado(folioservicio.ToString().Trim(),aplica_iva_servmedico);
				NpgsqlConnection conexion; 
				conexion = new NpgsqlConnection (connectionString+nombrebd);
			    // Verifica que la base de datos este conectada
				try{
					...
					comando = conexion.CreateCommand ();
			 		comando.CommandText = "UPDATE osiris_erp_cobros_enca "+
											...
											"reservacion = 'false',"+
											"subtotal15 = '"+entry_subtotal_al_15.Text.Trim()+"',"+
											"subtotal0 = '"+entry_subtotal_al_0.Text.Trim()+"',"+
											"subtotal_iva = '"+entry_total_iva.Text.Trim()+"',"+
											"total_costo_con_impuesto = '"+subtotalcon_impuesto_costo.ToString().Trim()+"',"+
											"total_costo_sin_impuesto = '"+subtotalsin_impuesto_costo.ToString().Trim()+"',"+
											"total_iva_costo = '"+totalivacosto.ToString().Trim()+"',"+
		...
		{
			if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_abrir_folio","WHERE acceso_abrir_folio = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_abrir_folio","bool") == "True"){
				MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
				...
				msgBox.Destroy();
		 		...
		 		if (miResultado == ResponseType.Yes){
					...
					try{
						...
						llenado_de_productos_aplicados(folioservicio.ToString().Trim(),false);
				    }catch (NpgsqlException ex){
					   	MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
											MessageType.Error, 
											ButtonsType.Close,"PostgresSQL error: {0}",ex.Message);
		...
		void on_button_compara_extras_clicked(object sender, EventArgs args)
		{
			if (this.folioservicio == 0 ){	
				...
				new osiris.validacion_cargos_extras(folioservicio.ToString().Trim());	
			}			
		}
			...
			if (this.folioservicio == 0 ){	
				...
				new osiris.abonos(PidPaciente,this.folioservicio,nombrebd,
						...
						dir_pac,cirugia,empresapac,id_tipopaciente,NomEmpleado+" "+AppEmpleado+" "+ApmEmpleado,LoginEmpleado,
				                  agregarmasabonos,montoconvenio.ToString(),cuenta_cerrada,tienepagare);
			}
		}
		...
		void on_button_honorario_medico_clicked(object sender, EventArgs args)
		{
			...
			if (this.folioservicio == 0){	
				...
				entry_motivo_ingreso.Text = entry_diagnostico.Text;
				entry_dia_visitamed.Text = DateTime.Now.ToString("dd");
				entry_mes_visitamed.Text = DateTime.Now.ToString("MM");
				entry_ano_visitamed.Text = DateTime.Now.ToString("yyyy");
				
				//abre la ventana de busqueda de medicos
				button_busca_doctores.Clicked += new EventHandler(on_button_busca_doctores_clicked);
				if (agregarmashonorario == true){
				...
				button_agregar_honorario.Sensitive = !cuenta_cerrada;
				button_agregar_honorario.Clicked += new EventHandler(on_button_agregar_honorario_clicked);
				...
				button_salir.Clicked += new EventHandler(on_cierraventanas_clicked);
				llenado_combobox(0,"",combobox_tipo_honorario,"array","","","",args_tipohonormedico,args_id_array,"");
				crea_treeview_hmedicos();
				llenado_de_honorarios();
			}
		}

		void crea_treeview_hmedicos()
		{
			treeViewEngineHonorarios = new TreeStore( typeof(string),typeof(string),typeof(string),typeof(string),
													typeof(string),typeof(bool),typeof(string),typeof(string),
		...
		void llenado_de_honorarios()
		{
			...
			try{
				...
				comando.CommandText = "SELECT to_char(osiris_erp_honorarios_medicos.id_medico,'999999') AS idmedico, "+ 
									...
									"to_char(id_abono,'999999999') AS idabono,id_tipo_honorario,"+
				...
				while(lector.Read()){
					...
					treeViewEngineHonorarios.AppendValues((string) lector["idmedico"],
														...
														true,
														lector["id_tipo_honorario"].ToString().Trim());
				}
		...
		{
			MessageDialog msgBoxError;
			TreeIter iter;
			TreeModel model;
			if (lista_de_honorarios.Selection.GetSelected (out model, out iter)){
				if ((bool) lista_de_honorarios.Model.GetValue (iter,5) == false){
					idtipohonorario = int.Parse((string) lista_de_honorarios.Model.GetValue (iter,9));
					idhonorariomedico = (string) lista_de_honorarios.Model.GetValue (iter,7);
					if(idtipohonorario == 2){
						pagodehonorario = true;
						pagado_de_honorario_medico("0",idhonorariomedico);
						llenado_de_honorarios();
					}else{
						pagodehonorario = true;
						...
						entry_observacion_egreso.Text = "PAGO DE H.MEDICOS DR."+lista_de_honorarios.Model.GetValue (iter,1).ToString();
						entry_total_comprobante.Text = (string) lista_de_honorarios.Model.GetValue (iter,3);
						...
						button_salir.Clicked += new EventHandler(on_cierraventanas_clicked);
						llenado_combobox(0,"",combobox_formapago,"sql","SELECT * FROM osiris_erp_forma_de_pago WHERE proveedor = 'false' AND activo_formapago = 'true' ORDER BY descripcion_forma_de_pago;","descripcion_forma_de_pago","id_forma_de_pago",args_args,args_id_array,"");
						llenado_combobox(0,"",combobox_tipo_honorario,"array","","","",args_tipohonormedico,args_id_array,"");
						llenado_combobox(0,"",combobox_tipocomprobante,"sql","SELECT * FROM osiris_erp_tipo_comprobante WHERE activo = 'true' AND id_tipo_comprobante IN('1','7') ORDER BY id_tipo_comprobante;","descripcion_tipo_comprobante","id_tipo_comprobante",args_args,args_id_array,"");
					}
				}else{
					msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,MessageType.Info,ButtonsType.Close, "El Honorario Medico seleccionado ya esta pagado, verifique...");
					msgBoxError.Run ();	msgBoxError.Destroy();
				}
			}else{
				msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,MessageType.Info,ButtonsType.Close, "Seleccione un Honorario Medico");
				msgBoxError.Run ();	msgBoxError.Destroy();
			}
		}
 			...
 			if (lista_de_honorarios.Selection.GetSelected (out model, out iter)){ 				
				if((bool) this.lista_de_honorarios.Model.GetValue (iter,8)){					
					new osiris.rpt_solicitud_cheque((string) this.lista_de_honorarios.Model.GetValue (iter,1),
				                                (string) this.lista_de_honorarios.Model.GetValue (iter,3), 
					                                entry_tipo_paciente.Text.Trim(), 
					                                entry_aseguradora.Text.Trim(),
		...
		void on_selecciona_medico_clicked (object sender, EventArgs args)
		{
			...
			if (lista_de_medicos.Selection.GetSelected(out model, out iterSelected)){
				...
				entry_idmedico.Text = idmedico.ToString().Trim();
		...
		void on_button_agregar_honorario_clicked (object sender, EventArgs args)
		{
			Gtk.MessageDialog msgBox;
			float toma_valor = 0;
			if(entry_motivohonorario.Text.Trim() != ""){
				if(entry_valor_honorario.Text != ""){
					if(float.Parse(entry_valor_honorario.Text.ToString().Trim()) != 0){
						if(idtipohonorario != 0){
							toma_valor = float.Parse(entry_valor_honorario.Text);
							totalhonorarios += toma_valor;
							entry_totalbonos_medicos.Text = totalhonorarios.ToString();
							treeViewEngineHonorarios.AppendValues(entry_idmedico.Text.ToString().Trim(),
						  											nombmedico,
						  											especialidadmed,
						  											toma_valor.ToString(),
																	...
																	"",
																	false);
							entry_valor_honorario.Text = "0";
						}else{
							msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
											MessageType.Error,ButtonsType.Ok,"Debe especificar un tipo de Honorario Medico, verifique...");
							msgBox.Run ();				msgBox.Destroy();
						}
					}else{
						msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
											MessageType.Error,ButtonsType.Ok,"Verifique la cantidad del Honorario Medico...");
						msgBox.Run ();				msgBox.Destroy();
					}
				}else{
					msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
				...
				msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
											MessageType.Error,ButtonsType.Ok,"Debe poner el Motivo de Honorario, verifique...");
				msgBox.Run ();				msgBox.Destroy();
			}
		}
			...
			if (combobox_tipo_busqueda.GetActiveIter(out iter))
			{
				if((int) combobox_tipo_busqueda.Model.GetValue(iter,1) > 0) {
					...
					try{
						...
						while (lector.Read()){
							treeViewEngineMedicos.AppendValues ((int) lector["id_medico"],
										(string) lector["nombre1_medico"],
										(string) lector["nombre2_medico"],
										(string) lector["apellido_paterno_medico"],
										(string) lector["apellido_materno_medico"],
										(string) lector["descripcion_especialidad"],
										(string) lector["cedula_medico"],
										(string) lector["telefono1_medico"],
										(string) lector["telefono2_medico"],
										(string) lector["celular1_medico"],
										(string) lector["celular2_medico"],
										(string) lector["nextel_medico"],
										(string) lector["beeper_medico"],
							            (string) lector["descripcion_empresa"],
										(string) lector["idespecialidad"],
										(string) lector["idempresa"],
										(string) lector["fecha_ingreso"],
										(string) lector["fecha_revision"],
										(string) lector["direccion_medico"],
										(string) lector["direccion_consultorio_medico"],
										(bool) lector["titulo_profesional_medico"],
										(bool) lector["cedula_profecional_medico"],
										(bool) lector["diploma_especialidad_medico"],
										(bool) lector["diploma_subespecialidad_medico"],
										(bool) lector["copia_identificacion_oficial_medico"],
										(bool) lector["copia_cedula_rfc_medico"],
										(bool) lector["diploma_cursos_adiestramiento_medico"],
										(bool) lector["certificacion_recertificacion_consejo_subespecialidad_medico"],
										(bool) lector["copia_comprobante_domicilio_medico"],
										(bool) lector["diploma_seminarios_medico"],
										(bool) lector["diploma_cursos_medico"],
										(bool) lector["diplomas_extranjeros_medico"],
										(bool) lector["constancia_congresos_medico"],
										(bool) lector["cedula_especialidad_medico"],
										(bool) lector["medico_activo"],
										(bool) lector["centro_medico"],
										(bool) lector["autorizado"]);
						}
					}catch (NpgsqlException ex){
			   			Console.WriteLine ("PostgresSQL error: {0}",ex.Message);
			   			MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
		...
		{
			//new osiris.rpt_honorarios_medicos (query_fechas, nombrebd, LoginEmpleado, NomEmpleado, AppEmpleado, ApmEmpleado, "", orden, query_medico,query_nroatencion);
		}
		...
		void on_button_guardar_honorario_clicked(object sender, EventArgs args)
		{
			...
			msgBox = new MessageDialog (MyWin,DialogFlags.Modal,MessageType.Question,ButtonsType.YesNo,"¿ Esta seguro de guardar esta informacion ?");
			miResultado = (ResponseType)msgBox.Run ();
			msgBox.Destroy();
			    ...
			    try{
					...
					if (treeViewEngineHonorarios.GetIterFirst (out iter)){
						if ((bool)lista_de_honorarios.Model.GetValue (iter,8) == false){		
							comando.CommandText = "INSERT INTO osiris_erp_honorarios_medicos( "+
												...
												idtipohonorario.ToString().Trim()+"','"+
 												folioservicio+"','"+
 												DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"','"+
 												LoginEmpleado+"','"+
												entry_motivohonorario.Text.ToString().ToUpper()+"','"+
												id_tipopaciente.ToString().Trim()+"','"+
												entry_ano_visitamed.Text.ToString().Trim()+"-"+entry_mes_visitamed.Text.ToString().Trim()+"-"+entry_dia_visitamed.Text.ToString().Trim()+"','"+
												entry_motivo_ingreso.Text+"');";
		 					comando.ExecuteNonQuery();
					    	comando.Dispose();
				    ...
				    if(graboinformacion == true){
					    ...
					    try{
							...
							NpgsqlCommand comando1; 
							comando1 = conexion1.CreateCommand ();
							if(idtipohonorario == 1){
								comando1.CommandText = "UPDATE osiris_erp_cobros_enca "+
									"SET honorario_medico = honorario_medico + ('"+entry_totalbonos_medicos.Text.Trim()+"'*-1) "+
									"WHERE folio_de_servicio =  '"+folioservicio+"';";
							}
							if(idtipohonorario == 2){
								comando1.CommandText = "UPDATE osiris_erp_cobros_enca "+
									"SET honorario_medico = honorario_medico + '"+entry_totalbonos_medicos.Text.Trim()+"' "+
									"WHERE folio_de_servicio =  '"+folioservicio+"';";
							}
		...
		void on_quita_honorario_clicked (object sender, EventArgs args)
		{
			TreeIter iter;
 			...
 			if (lista_de_honorarios.Selection.GetSelected (out model, out iter)) {
 				...
 				if (!(bool) lista_de_honorarios.Model.GetValue (iter,8)){
 					totalhonorarios -= toma_honorario;
					entry_totalbonos_medicos.Text = totalhonorarios.ToString();
 					treeViewEngineHonorarios.Remove (ref iter);
				}else{
					Gtk.MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
										MessageType.Error,ButtonsType.Ok,"Este honorario NO se puede quitar ya que se encuentra grabado, verifique...");
					msgBox.Run ();					msgBox.Destroy();
				}
			}
		}		
		...
		{
			imprime_proc_por_fecha_totales("procedimiento","pdf");
		}
		
		// Imprime el resumen de la factura
		void on_button_procedimiento_totales_clicked(object sender, EventArgs args)
		{
			imprime_proc_por_fecha_totales("resumen_factura","pdf");
		}
		
		void imprime_proc_por_fecha_totales(string tipo_de_reporte,string tipo_salida_)
		{
			...
			if ((string) entry_folio_servicio.Text == "" || (string) entry_pid_paciente.Text == "" ){	
				...
				checkbutton_export_to.ChildVisible = false;
				...
				checkbutton_incluye_concep_ccj.Label = "Incluir Tot.X Depto.";
				label433.Text = "Tipo de Reporte";
				checkbutton_todos_rango_fecha.ChildVisible = false;
				entry_dia1.Text = fecha_rango_1.Substring(0,2);
				...
				entry_mes1.Text = fecha_rango_1.Substring(3,2);
				...
				entry_ano1.Text = fecha_rango_1.Substring(6,4);
				...
				entry_referencia_inicial.Text = fecha_rango_1 + " "+fecha_rango_2;
		...
		void on_button_imprime_rangofecha_clicked(object sender, EventArgs args)
		{
			...
			new osiris.rpt_proc_cobranza (PidPaciente,folioservicio,nombrebd,
						...
						cirugia,empresapac,id_tipopaciente,query,entry_habitacion.Text,entry_especialidad.Text,entry_diagnostico.Text,
			            tipodereporte,tipo_salida,idpaquetequirurgico,aplica_iva_servmedico);
		...
		void comprobante_venta_servicio()
		{
			if ((string) entry_folio_servicio.Text == "" || (string) entry_pid_paciente.Text == "" ){	
				...
				entry_numero_comprobante.KeyPressEvent += onKeyPressEvent;
				entry_total_comprobante.KeyPressEvent += onKeyPressEvent;
				entry_monto_convenio.KeyPressEvent += onKeyPressEvent;
				...
				entry_ano1.Text = DateTime.Now.ToString("yyyy");
				entry_hora_compr.Text = DateTime.Now.ToString("HH:mm:ss");
				entry_monto_convenio.Text = montoconvenio.ToString().Trim();
				
				entry_dia1.IsEditable = false;
				entry_mes1.IsEditable = false;
				entry_ano1.IsEditable = false;
				entry_hora_compr.IsEditable = false;
				
				llenado_combobox(0,"",combobox_tipocomprobante,"sql","SELECT * FROM osiris_erp_tipo_comprobante WHERE activo = 'true' AND acceso_caja_abonos = 'true' ORDER BY id_tipo_comprobante;","descripcion_tipo_comprobante","id_tipo_comprobante",args_args,args_id_array,"");
				llenado_combobox(0,"",combobox_formapago,"sql","SELECT * FROM osiris_erp_forma_de_pago WHERE proveedor = 'false' AND activo_formapago = 'true' ORDER BY clave_sat;","descripcion_forma_de_pago","id_forma_de_pago",args_args,args_id_array,"");
		...
		void guarda_comprobante(object sender, string tipo_de_comprobante)
		{
			...
			string descrippago = "";
			string montoafacturar = "0";
			string valorcomprobante = entry_total_comprobante.Text;
			
			if(idtipocomprobante != "1"){
				// Documento X facturar
				if(idtipocomprobante == "6" ){
					montoafacturar = entry_total_comprobante.Text.Trim();
					valorcomprobante = "0";
				}
				if (entry_numero_comprobante.Text.Trim() != "" && idformadepago > 1){								
					...
					if (idformadepago > 1 ){
						...
						msgBox.Destroy();
		 				...
		 				if (miResultado == ResponseType.Yes){				
							if(tipo_de_comprobante == "HONORARIO"){
								pago_sino = true;
								descrippago = entry_observacion_egreso.Text;
								...
								pagado_de_honorario_medico(entry_numero_comprobante.Text,idhonorariomedico);
								...
								sql_abonos_servicio = "INSERT INTO osiris_erp_abonos("+
													...
													"id_tipo_comprobante) "+
													"VALUES ('-"+
													entry_total_comprobante.Text.Trim()+"','"+
													entry_total_comprobante.Text.Trim()+"','"+
					 								folioservicio+"','"+
													...
													tipo_de_comprobante+"','"+
													id_tipopaciente.ToString().Trim()+"','"+
													idtipocomprobante.ToString().Trim()+ "');";
							...
							if(tipo_de_comprobante == "CAJA"){
								...
								sql_abonos_servicio = "INSERT INTO osiris_erp_abonos("+
					 							...
					 							"VALUES ('"+
					 								valorcomprobante+"','"+
													entry_monto_convenio.Text.Trim()+"','"+
					 								folioservicio+"','"+
									...
									if(idtipocomprobante != "6"){
										...
										try{
											...
											comando1 = conexion1.CreateCommand ();
								 			comando1.CommandText = "UPDATE osiris_erp_cobros_enca "+
												"SET pagado = 'true',"+	
												"total_pago = total_pago + '"+(string) entry_total_comprobante.Text.Trim()+"'," +
												"monto_convenio = '"+entry_monto_convenio.Text.Trim()+"' "+
												"WHERE  folio_de_servicio =  '"+folioservicio+"';";
											...
											try{
												...
												comando1 = conexion1.CreateCommand ();
								 				comando1.CommandText = "UPDATE osiris_erp_cobros_enca "+
																	...
																	"pagare = 'true',"+
																	"monto_convenio = '"+montoconvenio.ToString().Trim() +"' "+							
																	"WHERE folio_de_servicio = '"+folioservicio+"';";
												//Console.WriteLine(comando1.CommandText);
								 				comando1.ExecuteNonQuery();    	    	       	comando1.Dispose();
												Widget win = (Widget) sender;
				...
				MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
					MessageType.Error,ButtonsType.Close, "NO Selecciono un tipo de Comprobante, verifique...");
					msgBoxError.Run ();				msgBoxError.Destroy();
			}
		}
			...
			if((bool) tienepagare){
				...
				try{
					...
					comando.CommandText = "INSERT INTO osiris_erp_gestcobrzmov(" +
								...
								PidPaciente.ToString().Trim()+"','"+
								DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"','"+
								"SE GENERO UN ABONO EN EL ESTADO DE CUENTA DE $"+entry_total_comprobante.Text.Trim()+"','"+
								entry_telefono_paciente.Text.Trim()+"','"+
								id_tipopaciente.ToString().Trim()+"')";
		...
		void pagado_de_honorario_medico(string numerocomprobante,string idhonorariomedico)
		{
			...
			try{
				...
				comando1.CommandText = "UPDATE osiris_erp_honorarios_medicos "+
					...
					"fechahora_pagado ='"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"',"+
					"id_quien_pago = '"+LoginEmpleado.ToString().Trim()+"',"+ 
					"numero_recibo_caja = '"+numerocomprobante+"' "+				
					"WHERE id_abono = '"+idhonorariomedico.Trim()+"';";
				//Console.WriteLine(comando1.CommandText);
				comando1.ExecuteNonQuery();    	    	       	comando1.Dispose();											 			 	  
			}catch(NpgsqlException ex){
				MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
		...
		void comprobante_de_caja_pago(string tipo_comprobante,int numerodecomprobante)
		{
			...
			if (tipo_comprobante == "CAJA"){
				new caja_comprobante (numerodecomprobante, "CAJA", folioservicio,"SELECT osiris_erp_cobros_deta.folio_de_servicio AS foliodeservicio,osiris_erp_cobros_deta.pid_paciente AS pidpaciente, "+ 
						...
						"osiris_productos.id_grupo_producto,osiris_erp_cobros_deta.porcentage_iva, "+
					    ...
					    "osiris_erp_abonos.concepto_del_abono AS concepto_comprobante,osiris_erp_tipo_comprobante.id_tipo_comprobante,"+
						"osiris_erp_cobros_enca.id_empresa,descripcion_empresa," +
						"osiris_erp_cobros_enca.id_aseguradora,descripcion_aseguradora," +
						"osiris_erp_cobros_enca.nombre_medico_encabezado,osiris_erp_cobros_enca.nombre_medico_tratante,"+
				        ...
				        "AND osiris_erp_abonos.id_quien_creo = osiris_empleado.login_empleado "+
			...
			if (tipo_comprobante == "PAGARE"){			
				new caja_comprobante (numerodecomprobante, "PAGARE", folioservicio,"SELECT osiris_erp_cobros_enca.folio_de_servicio AS foliodeservicio,osiris_erp_cobros_enca.pid_paciente AS pidpaciente," +
						    ...
						    "to_char(osiris_erp_cobros_enca.fechahora_creacion,'dd-mm-yyyy') AS fechcreacion,  "+
							"to_char(osiris_erp_cobros_enca.fechahora_creacion,'HH:mi') AS horacreacion,osiris_erp_tipo_comprobante.id_tipo_comprobante,"+
		...
		void crea_treeview_servicio()
		{
			...
			lista_de_servicios.RowActivated += on_edita_registro;
			...
			col_cantidad_hc.Resizable = true;
		...
		void on_edita_registro(object sender, EventArgs args)
		{
			TreeModel model;
			TreeIter iterSelected;
			if (lista_de_servicios.Selection.GetSelected(out model, out iterSelected)){
				if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_editar_concepto","WHERE acceso_editar_concepto = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_editar_concepto","bool") == "True"){
					Glade.XML gxml = new Glade.XML (null, "caja.glade", "edita_concepto", null);
					gxml.Autoconnect (this);
					edita_concepto.Show();
					//entry_numero_regedit.Hide();
					//entry_cantapli_regedit.Hide();
					
					button_graba_editconcep.Clicked += new EventHandler(on_button_graba_editconcep_clicked);
					button_salir.Clicked += new EventHandler(on_cierraventanas_clicked);
					entry_monto_edit.KeyPressEvent += onKeyPressEvent;
					
					entry_folio_servicio_edit.Text = folioservicio.ToString().Trim();
					entry_pid_paciente_edit.Text = PidPaciente.ToString().Trim();
					entry_nombre_paciente_edit.Text = entry_nombre_paciente.Text;
					entry_descrip_concepto.Text = (string) model.GetValue(iterSelected, 0);
					entry_monto_actual.Text = float.Parse((string) model.GetValue(iterSelected, 6)).ToString("F");
					entry_valoriva_regedit.Text  = float.Parse((string) model.GetValue(iterSelected, 5)).ToString("F");
					
					entry_cantapli_regedit.Text = model.GetValue(iterSelected, 1).ToString();
					entry_numero_regedit.Text = (string) model.GetValue(iterSelected, 18);
					entry_folio_servicio_edit.ModifyBase(StateType.Normal, new Gdk.Color(246,193,119));		// Color Naranja
					entry_pid_paciente_edit.ModifyBase(StateType.Normal, new Gdk.Color(246,193,119));		// Color Naranja
					entry_nombre_paciente_edit.ModifyBase(StateType.Normal, new Gdk.Color(246,193,119));	// Color Naranja
					
					statusbar_edita_concepto.Pop(0);
					statusbar_edita_concepto.Push(1, "login: "+LoginEmpleado+"  |Usuario: "+NomEmpleado+" "+AppEmpleado+" "+ApmEmpleado);
					statusbar_edita_concepto.HasResizeGrip = false;
				}else{
					MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
		...
		void on_button_graba_editconcep_clicked(object sender, EventArgs args)
		{
			
			if((string) entry_monto_edit.Text != ""){
				Gtk.MessageDialog msgBox = new MessageDialog (MyWinError,DialogFlags.Modal,MessageType.Question,
													ButtonsType.YesNo,"¿ Esta seguro(a) de ACTUALIZAR la informacion ?");
				ResponseType miResultado = (ResponseType)msgBox.Run ();
				msgBox.Destroy();
				
				float iva_producto_ = 0;
				float precio_unitario_prod = 0;
				if(float.Parse(entry_valoriva_regedit.Text) != 0){
					precio_unitario_prod = (float.Parse((string) entry_monto_edit.Text)/float.Parse("1.16"))/float.Parse((string) entry_cantapli_regedit.Text);
					iva_producto_ = precio_unitario_prod * float.Parse("0.16");
				}else{
					precio_unitario_prod = float.Parse((string) entry_monto_edit.Text)/float.Parse((string) entry_cantapli_regedit.Text);
				}

				//Console.WriteLine("{0}, {1}, {3}",precio_unitario_prod,iva_producto_,entry_valoriva_regedit.Text);
				if (miResultado == ResponseType.Yes){
					string[,] parametros = {
						{ "precio_producto = ", "'"+precio_unitario_prod.ToString().Trim() +"'," },
						{ "iva_producto = ", "'" + iva_producto_.ToString().Trim() + "' " },
						{ "WHERE id_secuencia = '",entry_numero_regedit.Text.ToString().Trim()+ "';" }
					};
					object[] paraobj = {entry_folio_servicio };
					new osiris.update_registro ("osiris_erp_cobros_deta", parametros, paraobj);

					/*
					// Marcando registro para su eliminacion
					comando.CommandText = "UPDATE osiris_erp_cobros_deta "+
												"SET precio_producto = '"+precio_unitario_prod.ToString()+"',"+
												"iva_producto = '"+iva_producto_.ToString()+"' "+
												"WHERE id_secuencia = '"+entry_numero_regedit.Text.ToString().Trim()+"';";
					comando.ExecuteNonQuery();
					comando.Dispose();
					*/
		...
		*/
					...
					if ((int) folioservicio > 0){  // Validando que seleccione un folio de atencion
						if (treeViewEngineServicio.GetIterFirst (out iter)){
							if ((bool)lista_de_servicios.Model.GetValue (iter,14)==false){
								parametros = new [,] {{"id_producto,","'"+lista_de_servicios.Model.GetValue(iter,2).ToString().Trim()+"',"},
									{"folio_de_servicio,","'"+folioservicio.ToString().Trim()+"',"},
													{"pid_paciente,","'"+entry_pid_paciente.Text.ToString().Trim()+"',"},
													{"cantidad_aplicada,","'"+lista_de_servicios.Model.GetValue(iter,1).ToString().Trim()+"',"},
													{"id_tipo_admisiones,","'"+lista_de_servicios.Model.GetValue(iter,13).ToString().Trim()+"',"},
													{"precio_producto,","'"+lista_de_servicios.Model.GetValue(iter,3).ToString().Trim()+"',"},
													{"iva_producto,","'"+lista_de_servicios.Model.GetValue(iter,5).ToString().Trim()+"',"},
													{"precio_costo_unitario,","'"+lista_de_servicios.Model.GetValue(iter,15).ToString().Trim()+"',"},
													{"porcentage_utilidad,","'"+lista_de_servicios.Model.GetValue(iter,16).ToString().Trim()+"',"},
													{"porcentage_descuento,","'"+lista_de_servicios.Model.GetValue(iter,7).ToString().Trim()+"',"},
													{"id_empleado,","'"+LoginEmpleado+"',"},
													{"fechahora_creacion,","'"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"',"},
													{"porcentage_iva,","'"+valoriva+"',"},
													{"precio_costo,","'"+lista_de_servicios.Model.GetValue(iter,17).ToString().Trim()+"',"},
													{"id_referido,","'"+idmedico_ref.ToString().Trim()+"',"},
								...
								// actualizando el inventario del cargo
								parametros = new [,] {{ "stock = stock - ", "'"+float.Parse(lista_de_servicios.Model.GetValue(iter,1).ToString().Trim()).ToString()+"' " },
														{ "WHERE id_almacen = '", idsubalmacen.ToString()+ "' " },
														{ "AND id_producto = '", lista_de_servicios.Model.GetValue(iter,2).ToString().Trim()+ "';" }};
								paraobj = new [] { entry_folio_servicio };
	    	       			...
	    	       			while (treeViewEngineServicio.IterNext(ref iter)){
								...
								/*
									comando.CommandText = "INSERT INTO osiris_erp_cobros_deta("+
										...
										"id_almacen" +
										") VALUES ('"+
										lista_de_servicios.Model.GetValue(iter,2).ToString().Trim()+"','"+
										folioservicio+"','"+
										entry_pid_paciente.Text+"','"+
										lista_de_servicios.Model.GetValue(iter,1).ToString().Trim()+"','"+
										lista_de_servicios.Model.GetValue(iter,13).ToString().Trim()+"','"+
										lista_de_servicios.Model.GetValue(iter,3).ToString().Trim()+"','"+
										lista_de_servicios.Model.GetValue(iter,5).ToString().Trim()+"','"+
										lista_de_servicios.Model.GetValue(iter,15).ToString().Trim()+"','"+
										lista_de_servicios.Model.GetValue(iter,16).ToString().Trim()+"','"+
										lista_de_servicios.Model.GetValue(iter,7).ToString().Trim()+"','"+
										LoginEmpleado+"','"+
										DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"','"+
										valoriva+"','"+
										lista_de_servicios.Model.GetValue(iter,17).ToString().Trim()+"','"+
 					...
 					llenado_de_productos_aplicados( (string) entry_folio_servicio.Text,false );
 				}
 			}else{
 				MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
							MessageType.Info, 
				...
				//if ((int) lista_de_servicios.Model.GetValue (iter,13) != 400 ){  				
					if (!(bool) lista_de_servicios.Model.GetValue (iter,14)){
						...
						entry_subtotal_al_15.Text = subtotal_al_15.ToString("F");
	 					...
	 					if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_quitar_concepto","WHERE acceso_quitar_concepto = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_quitar_concepto","bool") == "True"){
	 						MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
							...
							msgBox.Destroy();
		 					...
		 					if (miResultado == ResponseType.Yes){
								...
								try{
									...
									// Rebajando del Inventario si se cargo por solicitud
									comando.CommandText = "UPDATE osiris_catalogo_almacenes SET stock = stock + '"+(float) lista_de_servicios.Model.GetValue(iter,1)+"' "+
													"WHERE id_almacen = '"+(string) lista_de_servicios.Model.GetValue (iter,19)+"' "+
													"AND id_producto = '"+lista_de_servicios.Model.GetValue(iter,2).ToString().Trim()+"' ;";
									comando.ExecuteNonQuery();
									comando.Dispose();
		...
		void llenado_de_productos_aplicados(string foliodeserv, bool muestra_mensajes)
		{
			subtotal_al_15 = 0;
			subtotal_al_0 = 0;
			total_iva = 0;
			...
			try{
				...
				comando.CommandText = "SELECT osiris_erp_cobros_enca.folio_de_servicio AS foliodeatencion, "+
				            	...
				            	"to_char(osiris_erp_cobros_enca.fecha_alta_paciente,'dd-MM-yyyy HH24:mi') AS fecha_egreso,"+
				            	...
				            	"osiris_erp_cobros_enca.numero_factura AS numfactura,"+
				            	...
				            	"osiris_erp_cobros_enca.id_medico,nombre_medico,nombre_medico_encabezado," +
				            	"osiris_erp_cobros_enca.id_medico_tratante,nombre_medico_tratante,"+
				            	"osiris_erp_movcargos.descripcion_diagnostico_movcargos,"+
				            	"osiris_erp_cobros_enca.facturacion, "+
				            	"osiris_erp_cobros_enca.pagado,monto_convenio,id_referido,osiris_erp_cobros_enca.pagare,id_paquete_quirurgico,"+
				            	...
				            	"osiris_his_tipo_pacientes.lista_de_precio AS listaprecio_tipopx," +
				            	"osiris_his_tipo_pacientes.sub_lista_precios AS sublistaprecio_tipopx "+
						
				            	"FROM "+ 
				            	"osiris_erp_cobros_enca,osiris_his_paciente,osiris_erp_movcargos,osiris_his_tipo_admisiones,osiris_his_tipo_pacientes, "+
				            	"osiris_his_habitaciones,osiris_aseguradoras,osiris_his_medicos,osiris_his_tipo_cirugias,osiris_his_tipo_especialidad,osiris_empresas "+
				            	"WHERE "+
				...
				if(lector.Read()){
					...
					button_solicitud_lab.Sensitive = true;
					button_solicitud_rx.Sensitive = true;
					button_solicitud_vision.Sensitive = true;
					button_solicitud_almacen.Sensitive = true;
					button_quitar_altapx.Sensitive = true;
					button_compara_extras.Sensitive = true;
					button_modifica_fechdeta.Sensitive = true;
								
					entry_ingreso.Text = lector["fecha_ingreso"].ToString().Trim();
					entry_egreso.Text = lector["fecha_egreso"].ToString().Trim();
					entry_numero_factura.Text = lector["numerofactura"].ToString().Trim();
					entry_nombre_paciente.Text = lector["nombre1_paciente"].ToString().Trim()+" "+lector["nombre2_paciente"].ToString().Trim()+" "+lector["apellido_paterno_paciente"].ToString().Trim()+" "+lector["apellido_materno_paciente"].ToString().Trim();
					...
					idmedico_ref = int.Parse((string) lector["id_referido"].ToString());
					idmedico_ingreso = (int) lector["id_medico"];
					idmedico_tratante = (int) lector["id_medico_tratante"];
					tienepagare = (bool) lector["pagare"];
					...
					aplica_precios_tipopx =  (bool) lector["listaprecio_tipopx"];
					aplica_precios_sub_tipopx =  (bool) lector["sublistaprecio_tipopx"];					
					...
					if(idaseguradora_paciente > 1){
						...
						if((string) classpublic.lee_registro_de_tabla("osiris_aseguradoras","servicio_medico_iva","WHERE servicio_medico_iva = 'true' AND id_aseguradora = '"+idaseguradora_paciente.ToString().Trim()+"' ","servicio_medico_iva","bool") == "True"){
							aplica_iva_servmedico = true;
						}
					}else{
						entry_aseguradora.Text = (string) lector["descripcion_empresa"];
						if((string) classpublic.lee_registro_de_tabla("osiris_empresas","servicio_medico_iva","WHERE servicio_medico_iva = 'true' AND id_empresa = '"+idempresa_paciente.ToString().Trim()+"' ","servicio_medico_iva","bool") == "True"){
							aplica_iva_servmedico = true;
						}
					}
					...
					fecha_nacimiento = (string) lector ["fecha_nac_pa"];
			       	cirugia = lector["descripcion_cirugia"].ToString().Trim();
					dir_pac = lector["direccion_paciente"].ToString().Trim()+" "+lector["numero_casa_paciente"].ToString().Trim()+" "+
            		...
            		entry_descrip_cirugia.Text = cirugia;
					entry_diagnostico.Text = lector ["descripcion_diagnostico_movcargos"].ToString().Trim();
					...
					empresapac = lector["descripcion_empresa"].ToString().Trim();
            		idpaquetequirurgico = (int) lector["id_paquete_quirurgico"];
					...
					numerofactura =  0;
					altamedicapaciente = (bool) lector["alta_paciente"];
            		...
            		if ((int) lector["idtipoadmision"] == 300 || (int) lector["idtipoadmision"] == 400  || 
						...
						(int) lector["idtipoadmision"] == 960 || (int) lector["idtipoadmision"] == 970 || (int) lector["idtipoadmision"] == 980){
	            		button_alta_paciente.Sensitive = true;	            		
	            	}else{
	            		button_alta_paciente.Sensitive = false;
	            		if (idhabitacion == 1){
							if(muestra_mensajes == true){
	            				MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
											MessageType.Error, 
								...
								if ((bool) lector ["pagado"] == false){
									...
									if ((bool) lector ["alta_paciente"] == false){										
										...
										if ((bool) lector ["bloqueo_de_folio"] == false){
											button_busca_producto.Sensitive = true;
											button_pase_quirofano.Sensitive = true;
											if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_bloqueo_cuenta","WHERE acceso_bloqueo_cuenta = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_bloqueo_cuenta","bool") == "True"){
												button_busca_producto.Sensitive = true;
											}else{
												button_cierre_cuenta.Sensitive = false;
												button_bloquea_cuenta.Sensitive = false;
					...
					llenado_de_material_aplicado(foliodeserv,aplica_iva_servmedico);						
				}else{
					MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
							MessageType.Error,ButtonsType.Close,"El Numero de Atencion Seleccionado  \n " +
																"NO EXISTE... verifique");
					msgBoxError.Run ();				msgBoxError.Destroy();
				}
		...
		void llenado_de_material_aplicado(string foliodeserv,bool aplica_iva_servmedico)
		{	
			...
			try{
				...
				comando.CommandText = "SELECT descripcion_producto,to_char(osiris_erp_cobros_deta.id_producto,'999999999999') AS idproducto, "+
							"to_char(osiris_erp_cobros_deta.cantidad_aplicada,'99999.99') AS cantidadaplicada,"+
							"to_char(osiris_erp_cobros_deta.precio_producto,'99999999.99') AS preciounitario,osiris_erp_cobros_deta.precio_costo_unitario,"+
							"to_char(osiris_erp_cobros_deta.cantidad_aplicada * osiris_erp_cobros_deta.precio_costo_unitario,'99999999.99') AS pcunitcantidad_,"+
							...
							"osiris_productos.id_grupo_producto,aplica_iva_engrupos, "+
							"osiris_erp_cobros_deta.id_tipo_admisiones AS idtipoadmision,"+
							"eliminado,osiris_productos.aplicar_iva,"+
							"osiris_erp_cobros_deta.id_secuencia AS secuencia "+
							"FROM osiris_erp_cobros_deta,osiris_productos,osiris_his_tipo_admisiones,osiris_grupo_producto "+
							"WHERE osiris_erp_cobros_deta.id_producto = osiris_productos.id_producto "+
							"AND osiris_erp_cobros_deta.id_tipo_admisiones = osiris_his_tipo_admisiones.id_tipo_admisiones " +
							"AND osiris_productos.id_grupo_producto = osiris_grupo_producto.id_grupo_producto "+
				...
				totalivacosto = 0;
				subtotalcon_impuesto_costo = 0;
				subtotalsin_impuesto_costo = 0;
				
				float tomasubtotalcon_impuesto_costo = 0;
				float tomasubtotalsin_impuesto_costo = 0;
				float tomasubtotal_al_15 = 0;
				float tomasubtotal_al_0 = 0;
					...
					if (!(bool) lector["eliminado"]){					
						...
						if ((bool) aplica_iva_servmedico == false){
							if ((bool) lector["aplicar_iva"] == true){	
								...
								calculo_del_iva_costo = (float.Parse((string) lector["pcunitcantidad_"])*float.Parse((string) lector["porcentageiva"]))/100;
								tomasubtotal_al_15 = float.Parse((string) lector["ppcantidad_"]);
								tomasubtotal_al_0 = 0;
								tomasubtotalcon_impuesto_costo = float.Parse((string) lector["pcunitcantidad_"].ToString().Trim());
								tomasubtotalsin_impuesto_costo = 0;								
							}else{
								calculo_del_iva_producto = 0;
								calculo_del_iva_costo = 0;
								tomasubtotal_al_15 = 0;
								tomasubtotal_al_0 = float.Parse((string) lector["ppcantidad_"]);
								tomasubtotalcon_impuesto_costo = 0;
								tomasubtotalsin_impuesto_costo = float.Parse((string) lector["pcunitcantidad_"].ToString().Trim());
							}							
						}else{
						...
						subtotal_al_15 += tomasubtotal_al_15;
						subtotal_al_0 += tomasubtotal_al_0;						
						subtotalcon_impuesto_costo += tomasubtotalcon_impuesto_costo;
						subtotalsin_impuesto_costo += tomasubtotalsin_impuesto_costo;						
						...
						totalivacosto = totalivacosto + calculo_del_iva_costo;
						...
						treeViewEngineServicio.AppendValues ((string) lector["descripcion_producto"],
														...
														(string) "",
														(string) "",
														(string) lector["secuencia"].ToString().Trim(),
														lector["id_almacen"].ToString().Trim());
					}
				}
				if (aplicar_descuento == false){
					totaldescuento = 0;
		...
		void on_button_lista_precios_clicked (object sender, EventArgs args)
		{
			busqueda = "lista_precio";
			Glade.XML gxml = new Glade.XML (null, "caja.glade", "busca_producto", null);
			gxml.Autoconnect (this);
			crea_treeview_busqueda("lista_precio");
		...
		void crea_treeview_busqueda(string tipo_busqueda)
		{
			if (tipo_busqueda == "paciente"){
				...
				col_Nombre1_Paciente.AddAttribute (cellrt2, "text", 2);
				...
				col_Nombre2_Paciente.AddAttribute (cellrt3, "text", 3);
				...
				col_app_Paciente.AddAttribute (cellrt4, "text", 4);
				...
				col_apm_Paciente.AddAttribute (cellrt5, "text", 5);
				...
				col_fechanacimiento_Paciente.AddAttribute (cellrt6, "text", 6);
				...
				col_edad_Paciente.AddAttribute (cellrt7, "text", 7);
				...
				col_sexo_Paciente.AddAttribute (cellrt8, "text", 8);
				...
				col_creacion_Paciente.AddAttribute (cellrt9, "text", 9);
			...
			if (tipo_busqueda == "medicos"){
				...
				col_nombrmedico.Title = "Nombre Medico";
				col_nombrmedico.PackStart(cellrt1, true);
				col_nombrmedico.AddAttribute (cellrt1, "text", 1);
	            
				TreeViewColumn col_espemedico = new TreeViewColumn();
				CellRendererText cellrt2 = new CellRendererText();
				col_espemedico.Title = "Especialidad";
				col_espemedico.PackStart(cellrt2, true);
				col_espemedico.AddAttribute (cellrt2, "text", 2);
	            
				TreeViewColumn col_telmedico = new TreeViewColumn();
				CellRendererText cellrt3 = new CellRendererText();
				col_telmedico.Title = "Telefono";
				col_telmedico.PackStart(cellrt3, true);
				col_telmedico.AddAttribute (cellrt3, "text", 3);
	            
				TreeViewColumn col_cedulamedico = new TreeViewColumn();
				CellRendererText cellrt4 = new CellRendererText();
				col_cedulamedico.Title = "Cedula Medica";
				col_cedulamedico.PackStart(cellrt4, true);
				col_cedulamedico.AddAttribute (cellrt4, "text", 4);
		...
		void on_selecciona_paciente_clicked(object sender, EventArgs args)
		{
			...
			TreeIter iterSelected;
 			...
 			if (lista_de_Pacientes.Selection.GetSelected(out model, out iterSelected)){
 				 ...
 				 llenado_de_productos_aplicados(folioservicio.ToString(),true);
 			}
 			// cierra la ventana despues que almaceno la informacion en variables
			Widget win = (Widget) sender;
			win.Toplevel.Destroy();
 		...
 		{
 			llenando_lista_de_productos(aplica_precios_tipopx);
 		}
 			
 		void llenando_lista_de_productos(bool aplica_precios_tipopx_)
 		{
			string acceso_a_grupos;
			if (busqueda != "lista_precio") {				
				if ((string)classpublic.lee_registro_de_tabla ("osiris_empleado", "acceso_todo_grupprod", "WHERE acceso_todo_grupprod = 'true' AND login_empleado = '" + LoginEmpleado + "' ", "acceso_todo_grupprod", "bool") == "True") {
					acceso_a_grupos = " ";
				} else {
					acceso_a_grupos = "AND osiris_productos.id_grupo_producto IN(" + classpublic.lee_registro_de_tabla ("osiris_almacenes", "id_almacen", " WHERE osiris_almacenes.id_almacen = '" + idsubalmacen.ToString ().Trim () + "' ", "acceso_grupo_prodcargos", "string") + ") ";
				}
			} else {
				acceso_a_grupos = "AND osiris_grupo_producto.agrupacion6 = 'CAJ' ";
			}
									...
									if(!error_de_captura){
										if ((string) entry_desc_producto.Text.Trim() == ""){}else{
											...
											foo = new Item (true,
												...
												id_produ,
												desc_produ,
												precio_produ.PadLeft(10),
												ppcantidad.ToString(),
												ivaproducto.ToString("F").PadLeft(10),
												suma_total.ToString("F").PadLeft(10),
												descuent_produ.PadLeft(10),
												valor_descuento.ToString("F"),
												preciocondesc.ToString("F").PadLeft(10),
												LoginEmpleado,
												DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
											...
											treeViewEngineExtras.AppendValues (true,
																...
																porcentage_utilidad_producto,
												costo_total_producto,0);
											//Console.WriteLine("costo_total_producto="+costo_total_producto.ToString().Trim()+ "    costotprodu = "+costotprodu.ToString().Trim());
										}
									}else{
										MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
										MessageType.Error, 
		...
		public void onKeyPressEvent_busqueda(object o, Gtk.KeyPressEventArgs args)
		{
			...
			if (args.Event.Key.ToString() == "Return" || args.Event.Key.ToString() == "KP_Enter"){
				...
				if(busqueda == "productos"){
					llenando_lista_de_productos(aplica_precios_tipopx);}
				if(busqueda == "lista_precio"){
					llenando_lista_de_productos(false);}
				if(busqueda == "medicos"){
					llenando_lista_de_medicos();} 		
		...
		struct Item
 		{
			...
			public int col_idsubalmacen{
				get { return col18_car_extr; }
			...
			public Item (bool col0_car_extr,float col1_car_extr,string col2_car_extr,string col3_car_extr,
					...
					string col12_car_extr,string col13_car_extr,int col14_car_extr,string col15_car_extr,
					string col16_car_extr,string col17_car_extr,int col18_car_extr )
			{
			...
			//if(button_busca_producto.Sensitive == true){
				if(button_solicitud_labrx.Name.ToString() == "button_solicitud_lab"){					
					new osiris.solicitudes_enfermeria(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,"Solicitud Estudio de LABORATORIO",400,"LAB",
					                                  "CAJA",id_tipopaciente,idempresa_paciente,idaseguradora_paciente,PidPaciente,folioservicio,
					                                  entry_nombre_paciente.Text.Trim(),idmedico_ingreso.ToString().Trim(),entry_doctor.Text.Trim(),
					                                  entry_descrip_cirugia.Text.Trim(),entry_habitacion.Text.Trim(),button_busca_producto.Sensitive,
				                                  	  aplica_precios_tipopx,aplica_precios_sub_tipopx,13);
				}
				...
				if(button_solicitud_labrx.Name.ToString() == "button_solicitud_optica"){
					new osiris.solicitudes_enfermeria(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,"Solicitud de OPTICA",970,"OPT",
					                                  "CAJA",id_tipopaciente,idempresa_paciente,idaseguradora_paciente,PidPaciente,folioservicio,
					                                  entry_nombre_paciente.Text.Trim(),idmedico_ingreso.ToString().Trim(),entry_doctor.Text.Trim(),
					                                  entry_descrip_cirugia.Text.Trim(),entry_habitacion.Text.Trim(),button_busca_producto.Sensitive,
				                                  aplica_precios_tipopx,aplica_precios_sub_tipopx,13);
				...
				if(button_solicitud_labrx.Name.ToString() == "button_solicitud_almacen"){
					new osiris.solicitud_material(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,idsubalmacen,folioservicio,PidPaciente,entry_nombre_paciente.Text);
				}
			//}else{
		...
		void on_button_solicitud_consulta_clicked(object obj, EventArgs args)
		{
			/*
			new osiris.interconsula_visimed(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,
				...
				entry_sexo_paciente.Text,this.idsubalmacen,this.entry_id_doctor.Text.Trim(),nombreespecialidad,nombreespecialidad,idtipointernamiento,false);
				*/
		}
		
		void crea_treeview_atenciones()
		{
			treeViewEngine_atenciones = new TreeStore(typeof (string),typeof (string),
							typeof (string), typeof (string), 
							typeof (string),typeof (string),typeof (string),
							typeof (string), typeof (string),typeof (string),
							typeof (string),typeof (bool));
	        							   
			treeview_atenciones.Model = treeViewEngine_atenciones;
			
			TreeViewColumn col_fecha = new TreeViewColumn();
			CellRendererText cellrt1 = new CellRendererText();
			col_fecha.Title = "Fecha-Hora";
			col_fecha.PackStart(cellrt1, true);
			col_fecha.AddAttribute (cellrt1, "text", 0);
			col_fecha.SortColumnId = (int) Column_servicio.col_fecha;
			col_fecha.Resizable = true;
			...
			col_servicio.SortColumnId = (int) Column_servicio.col_servicio;
			col_servicio.Resizable = true;
			...
			col_desc_servicio.SortColumnId = (int) Column_servicio.col_desc_servicio;
            col_desc_servicio.Resizable = true;
			...
			col_valor.SortColumnId = (int) Column_servicio.col_valor;
            col_valor.Resizable = true;
			...
			col_folio_ingreso.SortColumnId = (int) Column_servicio.col_folio_ingreso;
			col_folio_ingreso.Resizable = true;
			...
			col_num_factura.SortColumnId = (int) Column_servicio.col_num_factura;			
			col_num_factura.Resizable = true;
			...
			col_folio_ingreso_dep.SortColumnId = (int) Column_servicio.col_folio_ingreso_dep;
			col_folio_ingreso_dep.Resizable = true;
			...
			col_tipo_paciente.SortColumnId = (int) Column_servicio.col_tipo_paciente;
			col_tipo_paciente.Resizable = true;
			...
			col_empresaasegu.SortColumnId = (int) Column_servicio.col_empresaasegu;
			col_empresaasegu.Resizable = true;
			...
			col_admitio.SortColumnId = (int) Column_servicio.col_admitio;
			col_admitio.Resizable = true;
			...
			col_servmedico.SortColumnId = (int) Column_servicio.col_servmedico;
			col_servmedico.Resizable = true;
			...
			col_separacion.SortColumnId = (int) Column_servicio.col_separacion;
			col_separacion.Resizable = true;
                        
			treeview_atenciones.AppendColumn(col_fecha);
			treeview_atenciones.AppendColumn(col_servicio);
			treeview_atenciones.AppendColumn(col_desc_servicio);
			treeview_atenciones.AppendColumn(col_valor);
			treeview_atenciones.AppendColumn(col_folio_ingreso);
			treeview_atenciones.AppendColumn(col_num_factura);
			treeview_atenciones.AppendColumn(col_folio_ingreso_dep);
			treeview_atenciones.AppendColumn(col_tipo_paciente);
			treeview_atenciones.AppendColumn(col_empresaasegu);
			treeview_atenciones.AppendColumn(col_admitio);
			treeview_atenciones.AppendColumn(col_servmedico);
		 	treeview_atenciones.AppendColumn(col_separacion);
		}
		
		enum Column_servicio
		{
			col_fecha,
			col_servicio,
			col_desc_servicio,
		...
		{
			treeViewEngine_atenciones.Clear();
			NpgsqlConnection conexion; 
			conexion = new NpgsqlConnection (connectionString+nombrebd);
			//this.PidPaciente
			try{
				...
				while (lector.Read()){					
					...
					if (!(bool) lector["cancelado"]){ 
						treeViewEngine_atenciones.AppendValues ((string) lector["fechahoraadm"],
															(string) lector["descripcion_admisiones"],
															diagnostico_cirugia,
