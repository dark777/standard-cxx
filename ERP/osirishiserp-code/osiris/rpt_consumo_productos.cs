	...
	public class imprime_consumo_productos
	{
		...
		{	
			...
			if (treeViewEngineResumen.GetIterFirst (out iter)){
				...
				cr.MoveTo(550*escala_en_linux_windows,comienzo_linea*escala_en_linux_windows);			layout.SetText((string) lista_resumen_productos.Model.GetValue (iter,16));			Pango.CairoHelper.ShowLayout (cr, layout);
				cr.MoveTo(610*escala_en_linux_windows,comienzo_linea*escala_en_linux_windows);			layout.SetText(float.Parse((string) lista_resumen_productos.Model.GetValue (iter,17)).ToString().Trim());			Pango.CairoHelper.ShowLayout (cr, layout);
