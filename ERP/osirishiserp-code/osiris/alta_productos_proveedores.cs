...
namespace osiris
{
	public class alta_productos_proveedores
	{
		...
		Gtk.ListStore treeViewEngineaprobados = null;
		...
		Gtk.TreeStore treeViewEngineproveedores = null;
		Gtk.TreeStore treeViewEngineproductos = null;
		Gtk.TreeStore treeViewEngineBusca2 = null;
		...
		void onKeyPressEvent_enter_provedor(object o, Gtk.KeyPressEventArgs args)
		{
			if (args.Event.Key.ToString() == "Return" || args.Event.Key.ToString() == "KP_Enter"){				
				if((string) classpublic.lee_registro_de_tabla("osiris_erp_proveedores","descripcion_proveedor","WHERE id_proveedor = '"+entry_id_proveedor.Text+"' ","descripcion_proveedor","string") != ""){
		...
		void on_busca_provedor_clicked (object sender, EventArgs args)
		{
			...
			object[] parametros_objetos = {entry_id_proveedor,entry_nombre_proveedor,lista_precios_proveedor,treeViewEngineaprobados};
			...
			string[,] args_buscador2 = {{"ID PROVEEDOR","AND id_proveedor = '","' "},
										...
										{"RFC","AND rfc_proveedor = '","' "}};
			string[,] parametros_ordenpor = {{"DESCRIPCION PROVEEDOR","ORDER BY erp_proveedores.descripcion_proveedor",";"},
											{"ID PROVEEDOR","ORDER BY erp_proveedores.id_proveedor",";"}};
			classfind_data.buscandor(parametros_objetos,parametros_sql,parametros_string,"find_proveedores_catalogo_producto",0,args_buscador1,args_buscador2,parametros_ordenpor);
		}
		...
		{
			treeViewEngineaprobados = new ListStore(typeof(bool),
													     typeof(string),
													     typeof(string),
			                                             typeof(string),
			                                             typeof(string),
			...
			lista_precios_proveedor.Model = treeViewEngineaprobados;
			
			lista_precios_proveedor.RulesHint = true;
				
			lista_precios_proveedor.RowActivated += on_button_aprobar_clicked;
		...
		void on_togglebutton_editar_clicked (object sender, EventArgs args)
		{
			if(togglebutton_editar.Active == true){
				...
				try{
					...
					NpgsqlDataReader lector = comando.ExecuteReader ();
					if(lector.Read()){											
						secuencial = (string) lector["secuencia"];			
						entry_producto.Text = (string) lector["descripcion_producto"].ToString().ToUpper();
						entry_precio.Text = (string) lector["preciocosto"];
						entry_embalaje.Text = (string) lector["cantidadembalaje"];
						entry_codigo.Text = (string) lector["codigo_producto_proveedor"].ToString().ToUpper();
						entry_cod_barras.Text = (string) lector["codigo_de_barra"];		
						entry_clave.Text = (string) lector["clave"].ToString().ToUpper();
			...
			try{
				...
				comando = conexion.CreateCommand ();
               	comando.CommandText = "SELECT to_char(osiris_productos.id_producto,'999999999999') AS codProducto,"+
							"osiris_productos.descripcion_producto,osiris_productos.nombre_articulo AS nombrearticulo,osiris_productos.nombre_generico_articulo, "+
		...
		void on_selecciona_producto_clicked (object sender, EventArgs args)
		{
			...
			TreeModel model1;
			TreeIter iter1;			
			if(lista_de_producto.Selection.GetSelected(out model, out iter)){
				//Console.WriteLine("verifica el producto "+(string) model.GetValue(iter, 0));
				if(lista_precios_proveedor.Selection.GetSelected(out model1, out iter1)){
					lista_precios_proveedor.Model.SetValue(iter1,7,(string) model.GetValue(iter, 0));
					lista_precios_proveedor.Model.SetValue(iter1,0,true);
					lista_precios_proveedor.Model.SetValue(iter1,8,(string) model.GetValue(iter, 1));
					lista_precios_proveedor.Model.SetValue(iter1,6,(string) model.GetValue(iter, 13));				
					lista_precios_proveedor.Model.SetValue(iter1,5,(string) model.GetValue(iter, 15));
				}
			}			
