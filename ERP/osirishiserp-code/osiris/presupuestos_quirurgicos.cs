...
namespace osiris
{
	public class presupuestos_cirugias
	{
		...
		[Widget] Gtk.TextView entry_notas;
		...
		public presupuestos_cirugias(string LoginEmp, string NomEmpleado, string AppEmpleado, string ApmEmpleado, string nombrebd_ ) 
		{
			...
			entry_id_presupuesto.KeyPressEvent += onKeyPressEvent_enter_id;
			...
			button_graba_presupuesto.Clicked += new EventHandler(on_button_graba_presupuesto_clicked);
			button_buscar_cirugia.Clicked += new EventHandler(on_button_buscar_cirugia_clicked);
			button_busca_medico.Clicked += new EventHandler(on_button_busca_medico_clicked);
		...
		void on_button_procedimiento_cobrz_clicked(object sender, EventArgs args)
		{
			if ((string) entry_id_presupuesto.Text == "" ){	
				...
				new paquetes_reporte (int.Parse(entry_id_presupuesto.Text),entry_cirugia.Text,entry_medico.Text,
					...
					entry_id_presupuesto.Text.Trim(),entry_notas.Text.Trim().ToUpper(),true,"0");
					*/
			}
		}
		...
		void on_button_hoja_sin_precios_clicked(object sender, EventArgs args)
		{
			if ((string) entry_id_presupuesto.Text == "" ){	
				...
				new paquetes_reporte (int.Parse(entry_id_presupuesto.Text),entry_cirugia.Text,entry_medico.Text,
					nombrebd,"presupuestos",entry_deposito_minimo.Text.Trim(),entry_dias_internamiento.Text.Trim(),
					entry_tel_medico.Text.Trim(),entry_tel_opcional.Text.Trim(),entry_fax.Text.Trim(),
					entry_id_presupuesto.Text.Trim(),entry_notas.Text.Trim().ToUpper(),false,"0");*/
			}
		}
		...
		void on_button_buscar_cirugia_clicked(object sender, EventArgs args)
		{
			...
			object[] parametros_objetos = {entry_id_cirugia,entry_cirugia};
			...
			string[,] args_orderby = {{"",""}};
			classfind_data.buscandor(parametros_objetos,parametros_sql,parametros_string,"find_cirugia_presupuesto",0,args_buscador1,args_buscador2,args_orderby);
		}
		...
		{
			/*
			object[] parametros_objetos = {entry_id_doctor,entry_nombre_doctor};
