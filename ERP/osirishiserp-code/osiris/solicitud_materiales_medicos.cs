...
namespace osiris
{
	public class solicitud_material
	{
		...
		public solicitud_material(string LoginEmp_, string NomEmpleado_, string AppEmpleado_, string ApmEmpleado_, string nombrebd_,int idalmacen_,
		                          int folioservicio_,int PidPaciente_,string nombrepaciente_) 
		{
			...
			if(folioservicio_ != 0){
				checkbutton_sol_parastock.Sensitive = false;
				// verificando que el numero de atencion no tenga alta medica
				if((string) classpublic.lee_registro_de_tabla("osiris_erp_cobros_enca","alta_paciente","WHERE folio_de_servicio = '"+folioservicio_.ToString().Trim()+"' AND alta_paciente = 'false' ","alta_paciente","bool") == "False"){
					checkbutton_nueva_solicitud.Active = true;
					checkbutton_nueva_solicitud.Sensitive = true;
				}else{
					checkbutton_nueva_solicitud.Sensitive = false;
				}				
				button_busca_paciente.Sensitive = false;
				entry_folio_servicio.Text = folioservicio_.ToString().Trim();
				entry_pid_paciente.Text = PidPaciente_.ToString().Trim();
				entry_nombre_paciente.Text = nombrepaciente_.Trim();						
			}
		...
		void on_button_rpt_surtido_clicked(object sender, EventArgs args)
		{
			...
			new osiris.rpt_envio_almacen(nombrebd,idalmacen);
		}
		...
		void on_button_selecciona_pq_clicked(object sender, EventArgs args)
		{
			...
			string[,] args_orderby = {{"",""}};
			classfind_data.buscandor(parametros_objetos,parametros_sql,parametros_string,"find_cirugia_paquetes_soliprod",0,args_buscador1,args_buscador2,args_orderby);
		}
			...
			if (checkbutton_nueva_solicitud.Active == true){
				...
				msgBox.Destroy();
		 		if (miResultado == ResponseType.Yes){
					if((string) classpublic.lee_registro_de_tabla("osiris_almacenes","id_almacen"," WHERE osiris_almacenes.id_almacen = '"+idalmacen.ToString().Trim()+"' ","pre_solicitudes","bool") == "True"){
						checkbutton_presolicitud.Sensitive = true;	
					}else{
						checkbutton_presolicitud.Sensitive = false;
					}
					if((string) classpublic.lee_registro_de_tabla("osiris_almacenes","id_almacen"," WHERE osiris_almacenes.id_almacen = '"+idalmacen.ToString().Trim()+"' ","solicitar_por_stock","bool") == "True"){
	    ...
	    {
	    	//if(cuenta_lineas != valida_lineas){
				MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
		 ...
		 void almacena_productos_solicitados()
		 {
			...
			if(checkbutton_nueva_solicitud.Active == true){			
				entry_numero_solicitud.Text = (string) classpublic.lee_ultimonumero_registrado("osiris_his_solicitudes_deta","folio_de_solicitud","WHERE id_almacen = '"+idalmacen.ToString().Trim()+"' ");
			}
				...
				if(tipodesolicitud.Trim() != ""){
					if (this.treeViewEngineSolicitud.GetIterFirst (out iter)){
						if ((bool) this.lista_produc_solicitados.Model.GetValue (iter,8) == false){
							string[] parametros = {entry_numero_solicitud.Text,(string) this.lista_produc_solicitados.Model.GetValue(iter,1),(string) this.lista_produc_solicitados.Model.GetValue(iter,7),
												(string) lista_produc_solicitados.Model.GetValue(iter,6),(string) lista_produc_solicitados.Model.GetValue(iter,2),DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
												LoginEmpleado,idalmacen.ToString(),entry_folio_servicio.Text.ToString().Trim(),entry_pid_paciente.Text.ToString().Trim(),checkbutton_sol_parastock.Active.ToString().ToLower(),
												checkbutton_presolicitud.Active.ToString().ToLower(),entry_nombre_paciente.Text.ToString().Trim().ToUpper(),entry_cirugia.Text.ToString().Trim().ToUpper(),
												entry_diagnostico.Text.ToString().Trim().ToUpper(),entry_observacion.Text.ToString().Trim().ToUpper(),tipodesolicitud,"false","2000-01-02 00:00:00",""};
							object [] paraobj = {};
							new osiris.insert_or_update_solimater(true,parametros,paraobj);
						}							
						while (this.treeViewEngineSolicitud.IterNext(ref iter)){
							if ((bool) this.lista_produc_solicitados.Model.GetValue (iter,8) == false){
								string[] parametros1 = {entry_numero_solicitud.Text,(string) this.lista_produc_solicitados.Model.GetValue(iter,1),(string) this.lista_produc_solicitados.Model.GetValue(iter,7),
													(string) this.lista_produc_solicitados.Model.GetValue(iter,6),(string) this.lista_produc_solicitados.Model.GetValue(iter,2),DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
													LoginEmpleado,idalmacen.ToString(),entry_folio_servicio.Text.ToString().Trim(),entry_pid_paciente.Text.ToString().Trim(),checkbutton_sol_parastock.Active.ToString(),
													checkbutton_presolicitud.Active.ToString(),entry_nombre_paciente.Text.ToString().Trim().ToUpper(),entry_cirugia.Text.ToString().Trim().ToUpper(),
													entry_diagnostico.Text.ToString().Trim().ToUpper(),entry_observacion.Text.ToString().Trim().ToUpper(),tipodesolicitud,"false","2000-01-02 00:00:00",""};
								object [] paraobj1 = {};
								new osiris.insert_or_update_solimater(true,parametros1,paraobj1);
							}
						}						
		 ...
		 void on_button_quitar_productos_clicked(object sender, EventArgs args)
		 {
			...
			TreeIter iterSelected;
 			if (lista_produc_solicitados.Selection.GetSelected(out model, out iterSelected)){
 				MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
	    ...
	    void llena_solicitud_material(string numerodesolicutud)
	    {
			...
			try{
				...
				if(lector.Read()){
					...
					tipodesolicitud = (string) lector["tipo_solicitud"];
					estatus_solicitud = (bool) lector["status"];
			...
			TreeIter iterSelected;
 			if (lista_de_producto.Selection.GetSelected(out model, out iterSelected)){
 				if ((float) float.Parse((string) entry_cantidad_aplicada.Text) > 0){					
					if((bool) valida_producto_solicitado((string) model.GetValue(iterSelected, 0)) == false){;					
		...
		bool valida_producto_solicitado(string idproducto_)
		{
			return false;
		}
		...
		{
			if((string) this.entry_numero_solicitud.Text.Trim() != ""){
				string query_sql = "SELECT osiris_his_solicitudes_deta.folio_de_solicitud,to_char(osiris_his_solicitudes_deta.id_producto,'999999999999') AS idproductos,"+
               						"to_char(cantidad_solicitada,'999999.999') AS cantidadsolicitada,tipo_solicitud,"+
               						"to_char(osiris_his_solicitudes_deta.precio_producto_publico,'999999999.99') AS precioproductopublico,"+
									"to_char(osiris_his_solicitudes_deta.costo_por_unidad,'999999999.99') AS costoporunidad,"+
									"to_char(osiris_his_solicitudes_deta.cantidad_solicitada * osiris_his_solicitudes_deta.costo_por_unidad,'999999999.999') AS totcostsolici,"+
               						"to_char(cantidad_autorizada,'999999.999') AS cantidadautorizada,osiris_productos.cantidad_de_embalaje,id_quien_autorizo, "+
               						"to_char(fechahora_solicitud,'dd-MM-yyyy') AS fechahorasolicitud,"+
               						"to_char(fechahora_autorizado,'dd-MM-yyyy') AS fechahoraautorizado,procedimiento_qx,diagnostico_qx,"+
               						"status,surtido,osiris_productos.descripcion_producto,"+
               						"to_char(osiris_his_solicitudes_deta.id_secuencia,'9999999999') AS idsecuencia,"+
									...
									"nombre1_paciente,nombre2_paciente,apellido_paterno_paciente,apellido_materno_paciente "+
               						"FROM osiris_his_solicitudes_deta,osiris_his_paciente,osiris_productos,osiris_grupo_producto,osiris_grupo1_producto,osiris_grupo2_producto "+
               						//"WHERE osiris_his_solicitudes_deta.pid_paciente IN ('29742','30851','30850') "+
									"WHERE osiris_his_solicitudes_deta.folio_de_solicitud = '"+entry_numero_solicitud.Text.Trim()+"' "+
               						"AND eliminado = 'false' "+
               						"AND osiris_his_paciente.pid_paciente = osiris_his_solicitudes_deta.pid_paciente "+
									"AND osiris_his_solicitudes_deta.id_producto = osiris_productos.id_producto "+
               						"AND osiris_productos.id_grupo_producto = osiris_grupo_producto.id_grupo_producto "+
									...
									"ORDER BY osiris_his_solicitudes_deta.id_secuencia;";
									//"AND surtido = 'true' "+
									//"ORDER BY osiris_his_solicitudes_deta.pid_paciente,osiris_his_solicitudes_deta.id_secuencia;";
				string[] args_names_field = {"folio_de_solicitud","idproductos","cantidadsolicitada","costoporunidad","totcostsolici","cantidadautorizada","descripcion_producto","descripcion_grupo_producto","descripcion_grupo1_producto","descripcion_grupo2_producto","nombre1_paciente","nombre2_paciente","apellido_paterno_paciente","apellido_materno_paciente"};
											
				string[] args_type_field = {"float","string","float","float","float","float","string","string","string","string","string","string","string","string"};
				...
				MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
														MessageType.Error,ButtonsType.Close, 
													"Seleccione una solicitud para exportar a .ods, verifique... ");
				msgBoxError.Run ();					msgBoxError.Destroy();
			}
		}
	}
	
	public class insert_or_update_solimater
	{
		...
		class_public classpublic = new class_public();
		/// <summary>
		/// Initializes a new instance of the <see cref="osiris.insert_or_update_solimater"/> class.
		/// </summary>
		/// <param name='insert_or_update'>
		/// Insert_or_update.
		/// </param>
		...
		/// Paraobj.
		/// </param>
		public insert_or_update_solimater(bool insert_or_update,string[] parametros,object[] paraobj)
		{
			connectionString = conexion_a_DB._url_servidor+conexion_a_DB._port_DB+conexion_a_DB._usuario_DB+conexion_a_DB._passwrd_user_DB;
			nombrebd = conexion_a_DB._nombrebd;
			if((bool) insert_or_update == true){
    	        ...
    	        try{
					...
					string sql_graba = "INSERT INTO osiris_his_solicitudes_deta("+
															"folio_de_solicitud,"+
															"id_producto,"+
					...
					comando1.CommandText = sql_graba;
