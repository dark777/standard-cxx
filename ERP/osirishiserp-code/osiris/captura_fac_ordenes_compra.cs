...
namespace osiris
{
	public class factura_orden_compra
	{
		...
		[Widget] Gtk.Entry entry_uuid = null;
		...
		[Widget] Gtk.ComboBox combobox_tipo_documento = null;
		[Widget] Gtk.Entry entry_subtotal_docu = null;
		[Widget] Gtk.Entry entry_iva_docu = null;
		[Widget] Gtk.Entry entry_total_docu = null;
		...
		string[] args_args = {""};
		string[] args_tipo_documento = {"","FACTURA","NOTA O REMISION",};
		...
		TreeViewColumn col_16;
		TreeViewColumn col_19;
		TreeViewColumn col_21;
		...
		public factura_orden_compra(string LoginEmp_,string nombreempleado_,string nombrebd_)
		{
			...
			entry_dia_fechaenalmac.Text = DateTime.Now.ToString("dd");
			entry_mes_fechaenalmac.Text = DateTime.Now.ToString("MM");
			entry_ano_fechaenalmac.Text = DateTime.Now.ToString("yyyy");		
			...
			llenado_combobox(0,"",combobox_tipo_documento,"array","","","",args_tipo_documento,args_id_array,"");
		...
		{
			if(entry_orden_de_compra.Text != "" && entry_orden_de_compra.Text != "0"){
				...
				if( resp == (int) ResponseType.Accept ){				
					...
					if(radiobutton_orden_compra.Active == true){
						LeerXML(select_file_xml.Filename,radiobutton_orden_compra);
					}
					if(radiobutton_requisicion.Active == true){
						LeerXML(select_file_xml.Filename,radiobutton_requisicion);
					}
					if(radiobutton_sin_orden.Active == true){
						LeerXML(select_file_xml.Filename,radiobutton_sin_orden);
						LeerXML_UUID(select_file_xml.Filename,radiobutton_requisicion);
					}
				}
				...
				MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
													MessageType.Error, 
													ButtonsType.Close, "No ha puesto un numero de DOCUMENTO VALIDO verifique...");
				msgBoxError.Run ();								msgBoxError.Destroy();
			}
		}
		
		void LeerXML(string filename_xml,object obj)
		{
			Gtk.RadioButton onRadioButtonChanged = (Gtk.RadioButton) obj;
			//Gtk.RadioButton tipo_de_documento = obj as Gtk.RadioButton;
			XmlTextReader reader_xml = new XmlTextReader(filename_xml);
			string serie_de_factura = "";
			string numero_de_factura = "";
			string idproveedor_oc = "";
			
			switch (onRadioButtonChanged.Name.ToString()){	
				case "radiobutton_orden_compra":
						...
						switch (reader_xml.NodeType){
							case XmlNodeType.Element:
								...
								if (reader_xml.MoveToContent() == XmlNodeType.Element && reader_xml.Name == "cfdi:Comprobante"){
									if (reader_xml.HasAttributes){
										if(null != reader_xml.GetAttribute("serie")){
											serie_de_factura = reader_xml.GetAttribute("serie");
										}
										if(null != reader_xml.GetAttribute("folio")){
											numero_de_factura = reader_xml.GetAttribute("folio");
										}
										entry_num_factura_proveedor.Text = serie_de_factura.Trim()+numero_de_factura.Trim();
										...
										entry_sub_total.Text = reader_xml.GetAttribute("subTotal");
										entry_total.Text = reader_xml.GetAttribute("total");
									}
								}
								if (reader_xml.MoveToContent() == XmlNodeType.Element && reader_xml.Name == "cfdi:Emisor"){
									if (reader_xml.HasAttributes){
										idproveedor_oc = (string) classpublic.lee_registro_de_tabla("osiris_erp_proveedores","rfc_proveedor","WHERE rfc_proveedor = '"+reader_xml.GetAttribute("rfc").Trim()+"'","id_proveedor","string");
										if(entry_id_proveedor.Text != idproveedor_oc){
											MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
													MessageType.Error, 
													ButtonsType.Close, "El PROVEEDOR enlasado al XML no es igual con la ORDEN DE COMPRA... verifique...");
											msgBoxError.Run ();								msgBoxError.Destroy();
											button_guardar.Sensitive = false;
										}else{											
								...
								if (reader_xml.MoveToContent() == XmlNodeType.Element && reader_xml.Name == "cfdi:Impuestos"){
									entry_iva.Text = reader_xml.GetAttribute("totalImpuestosTrasladados");
								}
								if (reader_xml.MoveToContent() == XmlNodeType.Element && reader_xml.Name == "tfd:TimbreFiscalDigital"){
									if (reader_xml.HasAttributes){
										entry_uuid.Text = reader_xml.GetAttribute("UUID");
									}
								}
								break;
							default:
				...
				case "radiobutton_requisicion":
							...
							case XmlNodeType.Element:
								...
								if (reader_xml.MoveToContent() == XmlNodeType.Element && reader_xml.Name == "cfdi:Emisor"){
									if (reader_xml.HasAttributes){
										...
										if(entry_id_proveedor.Text == ""){
											MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
													MessageType.Error, 
													ButtonsType.Close, "El PROVEEDOR enlasado al XML no EXISTE... verifique...");
											msgBoxError.Run ();								msgBoxError.Destroy();
											button_guardar.Sensitive = false;
										}else{
					...
					llenado_treeview_nro_factura((bool) button_guardar.Sensitive);
				
				break;
				case "radiobutton_sin_orden":
		...
		void LeerXML_UUID(string filename_xml,object obj)
		{
			Gtk.RadioButton onRadioButtonChanged = (Gtk.RadioButton) obj;
			//Gtk.RadioButton tipo_de_documento = obj as Gtk.RadioButton;
			XmlTextReader reader_xml = new XmlTextReader(filename_xml);
					...
					while (reader_xml.Read()){
						switch (reader_xml.NodeType){
							case XmlNodeType.Element:
								...
								if (reader_xml.MoveToContent() == XmlNodeType.Element && reader_xml.Name == "cfdi:Addenda"){
									if (reader_xml.HasAttributes){
		...
		{
			llenado_treeview_nro_factura(true);
		}
		
		void llenado_treeview_nro_factura(bool activa_guardar)
		{
			TreeIter iter2;
			button_guardar.Sensitive = activa_guardar;
			if(entry_num_factura_proveedor.Text.Trim() != ""){
				if(entry_uuid.Text.Trim() != ""){
					if (treeViewEngineListaProdRequi.GetIterFirst (out iter2)){
						...
						lista_productos_a_recibir.Model.SetValue(iter2,21,entry_uuid.Text.ToUpper());
					...
					MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
												MessageType.Error, 
												ButtonsType.Close, "Debe tener un UUID del SAT realizar la entrada... verifique...");
				msgBoxError.Run ();								msgBoxError.Destroy();
				}
			}else{
				MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
											...
											if (treeViewEngineListaProdRequi.GetIterFirst (out iter)){
												...
												if((bool) radiobutton_orden_compra.Active == true){
													...
													comando.CommandText = "UPDATE osiris_erp_requisicion_deta SET "+
									 									...
									 									"uuid = '" + entry_uuid.Text.Trim().ToUpper()+"',"+
												...
												if((bool) radiobutton_requisicion.Active == true){							
													...
													comando.CommandText = "UPDATE osiris_erp_requisicion_deta SET "+
																		...
																		"uuid = '" + entry_uuid.Text.Trim().ToUpper()+"',"+
									 									"costo_producto = '" +lista_productos_a_recibir.Model.GetValue(iter,6).ToString().Trim()+"',"+
									 									"costo_por_unidad = '" + lista_productos_a_recibir.Model.GetValue(iter,7).ToString().Trim()+"',"+
									 									"precio_costo_prov_selec = '" +lista_productos_a_recibir.Model.GetValue(iter,6).ToString().Trim()+"',"+
									 									"precio_unitario_prov_selec = '" + lista_productos_a_recibir.Model.GetValue(iter,7).ToString().Trim()+"',"+
									 									"precio_producto_publico = '" + lista_productos_a_recibir.Model.GetValue(iter,17).ToString().Trim()+"',"+
									 									"cantidad_de_embalaje = '" + lista_productos_a_recibir.Model.GetValue(iter,4).ToString().Trim()+"',"+
									 									"cantidad_ingreso_stock = '"+lista_productos_a_recibir.Model.GetValue(iter,5).ToString().Trim()+"',"+
									 									"factura_sin_orden_compra = 'true'," +
												...
												while (treeViewEngineListaProdRequi.IterNext(ref iter)){
													if((bool) radiobutton_sin_orden.Active == true){
														comando.CommandText = "INSERT INTO osiris_erp_requisicion_deta( "+
																			...
																			"true','"+
																			lista_productos_a_recibir.Model.GetValue(iter,6)+"','"+
																			lista_productos_a_recibir.Model.GetValue(iter,7)+"','"+
																			lista_productos_a_recibir.Model.GetValue(iter,13)+"','"+
																			lista_productos_a_recibir.Model.GetValue(iter,14)+"','"+
																			lista_productos_a_recibir.Model.GetValue(iter,17)+"','"+
																			lista_productos_a_recibir.Model.GetValue(iter,18)+"','"+
		...
		void on_radiobutton_clicked(object sender, EventArgs args)
		{
			...
			if(radiobutton_recive.Name == "radiobutton_orden_compra"){
				...
				entry_num_factura_proveedor.Text = "";
				if(entry_orden_de_compra.Text != "0"){
					llenado_orden_de_compra();;
				}
			}
		...
		void crea_treeview_capturafactura()
		{
			...
			cellrt4.Edited += NumberCellEdited_Packs;
			...
			col_21 = new TreeViewColumn();
			cellrt21 = new CellRendererText();
			col_21.Title = "UUID Folio Fiscal";
			col_21.PackStart(cellrt21, true);
			col_21.AddAttribute (cellrt21, "text", 21);
			col_21.SortColumnId = (int) col_productos_recibidos.col_21;
			//cellrt13.Editable = true;
			...
			lista_productos_a_recibir.AppendColumn(col_21);
		...
		void NumberCellEdited_Packs (object sender, EditedArgs args)
		{
			Gtk.TreeIter iter;
			...
			if (esnumerico == true){
				treeViewEngineListaProdRequi.GetIter (out iter, new Gtk.TreePath (args.Path));
				treeViewEngineListaProdRequi.SetValue(iter,(int) col_productos_recibidos.col_04,float.Parse((string) args.NewText).ToString("F"));
				treeViewEngineListaProdRequi.SetValue(iter,(int) col_productos_recibidos.col_05,float.Parse(Convert.ToString(float.Parse((string) args.NewText) * float.Parse((string) lista_productos_a_recibir.Model.GetValue(iter,3)))).ToString("F"));
			}		
		}
			...
			if(entry_orden_de_compra.Text != ""){
				...
				try{
					...
					comando.CommandText = "SELECT numero_orden_compra,id_proveedor,descripcion_proveedor,direccion_proveedor,"+
						"faxnextel_proveedor,contacto_proveedor,condiciones_de_pago,fechahora_creacion,subtotal_orden_compra,iva_orden_compra,total_orden_compra "+
						"FROM osiris_erp_ordenes_compras_enca " +
						"WHERE numero_orden_compra = '"+entry_orden_de_compra.Text.Trim()+"';";
					//Console.WriteLine(comando.CommandText);
					NpgsqlDataReader lector = comando.ExecuteReader ();							
					if (lector.Read()){
						...
						entry_subtotal_docu.Text = lector["subtotal_orden_compra"].ToString();
						entry_iva_docu.Text = lector["iva_orden_compra"].ToString();
						entry_total_docu.Text = lector["total_orden_compra"].ToString();
					}
					comando = conexion.CreateCommand ();				
					comando.CommandText = "SELECT osiris_erp_requisicion_deta.id_secuencia,cantidad_solicitada,to_char(osiris_erp_requisicion_deta.id_producto,'999999999999') AS codProducto," +
										"osiris_productos.descripcion_producto,cantidad_comprada," +
										"osiris_productos.costo_producto,osiris_productos.costo_por_unidad,numero_factura_proveedor," +
										...
										"osiris_erp_requisicion_deta.cantidad_de_embalaje,osiris_erp_requisicion_deta.id_proveedor,osiris_erp_requisicion_deta.uuid," +
									 	"osiris_erp_requisicion_deta.costo_producto AS costoprod_osiris,osiris_erp_requisicion_deta.costo_por_unidad," +
									 	"osiris_erp_requisicion_deta.precio_costo_prov_selec AS costoproducto,osiris_erp_requisicion_deta.precio_unitario_prov_selec AS costoporunidad "+
					...
					while (lector1.Read()){
						treeViewEngineListaProdRequi.AppendValues (false,
						                                           ...
						                                           float.Parse(Convert.ToString((decimal) lector1["cantidad_comprada"]).ToString()).ToString("F"),
						                                           "0.00",
						                                           ...
						                                           float.Parse(Convert.ToString((decimal) lector1["costoprod_osiris"]).ToString()).ToString("F"),
						                                           (string) lector1["descproducto_osiris"].ToString(),
						                                           (string) lector1["codProducto"],
						                                           (string) lector1["descrip_prod_prov"],
						                                           (string) lector1["codigo_producto_proveedor"].ToString(),
						                                           "",
						                                           "",
						                                          ...
						                                          "0",
						                                           ...
						                                           (string) lector1["uuid"].ToString().Trim());
		...
		void on_busca_proveedores_clicked(object sender, EventArgs args)
		{
			...
			string[,] args_buscador2 = {{"ID PROVEEDOR","AND id_proveedor = '","' "},
										...
										{"RFC","AND rfc_proveedor = '","' "}};
			string[,] args_orderby = {{"",""}};
			classfind_data.buscandor(parametros_objetos,parametros_sql,parametros_string,"find_proveedores_OC",0,args_buscador1,args_buscador2,args_orderby);
		}
			...
			if(entry_num_factura_proveedor.Text != ""  && entry_id_proveedor.Text != "" && entry_id_proveedor.Text != "1"){
				...
				if(llenavalores == false){
					...
					if (lista_productos_a_recibir.Selection.GetSelected(out model, out iter)){
						entry_expresion.Text = (string) model.GetValue(iter,9);
		...
		void on_selecciona_producto_prov_clicked(object sender, EventArgs args)
		{
			...
			if (lista_productos_a_recibir.Selection.GetSelected(out model, out iter)){
				if (lista_de_producto.Selection.GetSelected(out model1, out iter1)){
					...
					lista_productos_a_recibir.Model.SetValue(iter,11,(string) entry_producto_proveedor.Text.Trim().ToUpper());
