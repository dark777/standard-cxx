...
namespace osiris
{
	public class tesoreria
	{
		...
		[Widget] Gtk.Button button_rpt_factgeneradas = null;
		[Widget] Gtk.Button button_proc_facturados;
		[Widget] Gtk.Button button_proc_no_facturados;
		[Widget] Gtk.Button button_rpt_honorarios_medicos = null;
		...
		void on_button_caja_clicked(object sender, EventArgs args)
		{
			...
			new osiris.caja_cobro(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,16,1);
		}
		...
		{
			new osiris.solicitud_material(LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,nombrebd,16,0,0,"");
		}
		...
		{
			new osiris.rpt_no_ingreso_caja("pdf");
		}
		...
		void on_button_cortecaja_clicked(object sender, EventArgs args)
		{
			if((string) classpublic.lee_registro_de_tabla("osiris_empleado","exportar_cortecaja","WHERE exportar_cortecaja = 'true' AND login_empleado = '"+LoginEmpleado+"' ","exportar_cortecaja","bool") == "True"){
				new osiris.corte_de_caja (nombrebd, LoginEmpleado);
			}else{
		...
		{
			if((string) classpublic.lee_registro_de_tabla("osiris_empleado","exportar_pasesqx","WHERE exportar_pasesqx = 'true' AND login_empleado = '"+LoginEmpleado+"' ","exportar_pasesqx","bool") == "True"){
		...
		void on_button_reportes_clicked(object sender, EventArgs args)
		{
			...
			////// Muestra ventana de Glade
			reportes_caja.Show();
			button_rpt_factgeneradas.Clicked += new EventHandler(on_button_factgeneradas_clicked);
		...
		{
			new osiris.reporte_de_abonos(nombrebd,"abonos_pagos",LoginEmpleado,0,0);	
		}
		
		void button_rpt_facturas_pendientes_clicked(object sender, EventArgs args)
		{
			facturas_ = "AND osiris_erp_factura_enca.pagada = 'false' " ;
		...
		void on_button_rpt_honorarios_medicos_cliked(object sender, EventArgs args)
		{
			...
			button_salir.Clicked += new EventHandler(on_cierraventanas_clicked);
			button_busca_cliente.Clicked += new EventHandler(on_button_busca_medico_clicked);
			button_imprime_rangofecha.Clicked += new EventHandler(on_button_imprime_rangofecha_clicked);
			//checkbutton_incluye_concep_ccj.hide
		}

		void on_button_imprime_rangofecha_clicked(object sender, EventArgs args)
		{
			if (checkbutton_export_to.Active == true) {
				string query_fechas = " AND to_char(osiris_erp_honorarios_medicos.fecha_visita,'yyyy-MM-dd') >= '"+entry_ano1.Text+"-"+entry_mes1.Text+"-"+entry_dia1.Text+"' "+
					" AND to_char(osiris_erp_honorarios_medicos.fecha_visita,'yyyy-MM-dd') <= '"+entry_ano2.Text+"-"+entry_mes2.Text+"-"+entry_dia2.Text+"' ";
				...
				string query_sql = "SELECT osiris_erp_honorarios_medicos.folio_de_servicio AS folioservicio,osiris_erp_cobros_enca.pid_paciente AS expediente," +
					...
					"AND eliminado = 'false' " +
					query_fechas+
					"ORDER BY id_tipo_honorario,osiris_erp_honorarios_medicos.folio_de_servicio;";

				string[] args_names_field = {"folioservicio","expediente","nombre_paciente","procedimiento_admision","nombre_medico","descripcion_especialidad","motivo_visita","fecharegistro","fechavisita","tipopaciente","valorhonorario","id_tipo_honorario","pagado","fechapago","fechaingreso"};
				string[] args_type_field = {"float","float","string","string","string","string","string","string","string","string","float","float","string","string","string"};
				...
				if (checkbutton_impr_todo_proce.Active == true) { 
					...
					query_fechas = "AND to_char(osiris_erp_honorarios_medicos.fecha_visita,'yyyy-MM-dd') >= '" + entry_ano1.Text + "-" + entry_mes1.Text + "-" + entry_dia1.Text + "' " +
						"AND to_char(osiris_erp_honorarios_medicos.fecha_visita,'yyyy-MM-dd') <= '" + entry_ano2.Text + "-" + entry_mes2.Text + "-" + entry_dia2.Text + "' ";
				}
				if (radiobutton_fecha.Active == true) { 
					ordenrpt = "ORDER BY id_tipo_honorario,to_char(osiris_erp_honorarios_medicos.fecha_visita,'yyyy-MM-dd');";
				}
				if (radiobutton_cliente.Active == true) {	
					ordenrpt = "ORDER BY nombre_medico";
				}
				new osiris.rpt_honorarios_medicos (query_fechas, nombrebd, LoginEmpleado, NomEmpleado, AppEmpleado, ApmEmpleado, "",ordenrpt, query_medico,"");
			}
		}
...
//}
		...
		void on_button_factgeneradas_clicked(object sender, EventArgs args)
		{
			Glade.XML gxml = new Glade.XML  (null, "caja.glade", "rango_de_fecha", null);
			gxml.Autoconnect (this);  
			rango_de_fecha.Show();
		...
		{	
			...
			if (checkbutton_impr_todo_proce.Active == true) { 
				...
				if(facturados == "FACTURADOS"){
					query_fechas = "AND to_char(osiris_erp_factura_enca.fechahora_factura,'yyyy-MM-dd') >= '"+entry_ano1.Text+"-"+entry_mes1.Text+"-"+entry_dia1.Text+"' "+
									"AND to_char(osiris_erp_factura_enca.fechahora_factura,'yyyy-MM-dd') <= '"+entry_ano2.Text+"-"+entry_mes2.Text+"-"+entry_dia2.Text+"' ";
				}
			...
			if(facturados == "FACTURADOS") {
				...
				if (this.pagados == false){			
					bool pagados = true;
					new osiris.reporte_porcedimientos_facturados(pagados,rango1,rango2,query_fechas,nombrebd,LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,facturados,orden,query_cliente,(bool) checkbutton_export_to.Active);
				}else{
			...
			if (facturados == "NO FACTURADOS"){ 
				bool pagados = false;
				new osiris.reporte_porcedimientos_facturados(pagados,rango1,rango2,query_fechas,nombrebd,LoginEmpleado,NomEmpleado,AppEmpleado,ApmEmpleado,facturados,"","",(bool) checkbutton_export_to.Active);
			}
			if (facturados == "CERRADOS"){
