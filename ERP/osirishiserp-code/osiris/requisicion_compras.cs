...
namespace osiris
{
	public class requisicion_materiales_compras
	{
		...
		[Widget] Gtk.Window requisicion_materiales = null;
		[Widget] Gtk.CheckButton checkbutton_nueva_requisicion = null;
		[Widget] Gtk.Entry entry_requisicion = null;
		[Widget] Gtk.Entry entry_status_requisicion = null;
		[Widget] Gtk.Button button_selecciona_requisicion = null;
		[Widget] Gtk.Button button_busca_requisicion = null;
		...
		[Widget] Gtk.Button button_envio_compras = null;
		[Widget] Gtk.Button button_enviopara_autorizar = null;
		[Widget] Gtk.Button button_autorizar_compra = null;
		[Widget] Gtk.Button button_no_autoriza_prod = null;
		[Widget] Gtk.Button button_busca_proveedores1 = null;
		[Widget] Gtk.Button button_busca_proveedores2 = null;
		[Widget] Gtk.Button button_busca_proveedores3 = null;
		[Widget] Gtk.Button button_cancela_req = null;
	
		[Widget] Gtk.Entry entry_dia_requerida = null;
		[Widget] Gtk.Entry entry_mes_requerida = null;
		[Widget] Gtk.Entry entry_ano_requerida = null;
		
		[Widget] Gtk.ComboBox combobox_tipo_requisicion = null;
		...
		[Widget] Gtk.ComboBox combobox_tipo_admision2 = null;
		[Widget] Gtk.Entry entry_motivo = null;
		[Widget] Gtk.Entry entry_observaciones;
		[Widget] Gtk.Entry entry_solicitado_por = null;
		[Widget] Gtk.Entry entry_nombre_prodrequisado = null;		
		[Widget] Gtk.Entry entry_totalitems_productos = null;
		
		[Widget] Gtk.Button button_guardar_requisicion = null;
		[Widget] Gtk.TreeView lista_requisicion_productos = null;
		...
		[Widget] Gtk.Statusbar statusbar_almacen_requi = null;
		...
		float subtotal_requisicion = 0;
		float ivatotal_requisicion = 0;
		float totaltotal_requisicion = 0;
											  ...
											  string centrocosto_,int idcentrocosto_, string campoacceso_,int[] array_idtipoadmisiones_, int accesomodulo_)
		{
			...
			entry_requisicion.ModifyFont(fontdesc);	// Cambia el tipo de letra del Entry a negrita y Sans
			entry_requisicion.ModifyBase(StateType.Normal, new Gdk.Color(185,238,243)); 	// Color Celeste
			//entry_requisicion.
			entry_total.ModifyBase(StateType.Normal, new Gdk.Color(185,238,243)); 			// Color Celeste
			entry_iva.ModifyBase(StateType.Normal, new Gdk.Color(255,243,169)); 			// Color Amarillo
			entry_sub_total.ModifyBase(StateType.Normal, new Gdk.Color(255,243,169)); 		// Color Amarillo
			entry_total.ModifyFont(fontdesc);		// Cambia el tipo de letra del Entry a negrita y Sans
		...
		{
				if((string) classpublic.lee_registro_de_tabla("osiris_empleado","exportar_requisicion","WHERE exportar_requisicion = 'true' AND login_empleado = '"+LoginEmpleado+"' ","exportar_requisicion","bool") == "True"){
				string query_sql = "SELECT osiris_erp_requisicion_deta.id_requisicion,to_char(osiris_erp_requisicion_deta.id_producto,'999999999999') AS idproducto,"+
										...
										"osiris_productos.nombre_articulo AS nombrearticulo,to_char(osiris_productos.cantidad_de_embalaje,'9999.99') AS cantidadembalaje,"+
										"osiris_productos.tipo_unidad_producto,to_char(osiris_erp_requisicion_deta.numero_orden_compra,'9999999999') AS numeroordencompra,"+
				...
				string[] args_names_field = {"id_requisicion","cantidadsolicitada","idproducto","nombrearticulo","tipo_unidad_producto","cantidadembalaje","fecharequisado","quienrequiso","tipo_de_requi"};
				...
				if(enviadacompras == true){
		  			new osiris.rpt_requisicion_compras(this.nombrebd,entry_requisicion.Text,entry_status_requisicion.Text,entry_fecha_solicitud.Text,
				                                   ...
				                                   descripcion_tipo_requi,descripinternamiento,descripinternamiento2,entry_nombre_paciente.Text,entry_folio_servicio.Text,entry_pid_paciente.Text,
					                                   subtotal_requisicion,ivatotal_requisicion,totaltotal_requisicion);
				}else{
					MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
		...
		void on_button_rep_clicked(object sender, EventArgs args)
		{
			...
			if(checkbutton_export.Active == true){
				string query_sql = "SELECT id_requisicion,fechahora_creacion_requisicion,"+
									"osiris_erp_requisicion_enca.id_tipo_admisiones,osiris_his_tipo_admisiones.descripcion_admisiones AS departamento,"+
									...
									"osiris_erp_requisicion_enca.numero_orden_compra AS nro_orden_compra,"+
									"enviada_a_compras,fechahora_envio_compras,osiris_erp_requisicion_enca.observaciones AS observacion,motivo_requisicion,"+
									...
									"osiris_erp_requisicion_enca.id_tipo_requisicion_compra AS idtiporequicompra,osiris_erp_tipo_requisiciones_compra.descripcion_tipo_requisicion AS tipo_requisicion," +
									"sub_total_req,iva_req,total_req "+
				...
				string[] args_names_field = {"id_requisicion","fechacrearequisicion","departamento","enviada_a_compras","observacion","motivo_requisicion","tipo_requisicion","sub_total_req","iva_req","total_req","nro_orden_compra"};
				string[] args_type_field = {"float","string","string","string","string","string","string","float","float","float","string"};
				string[] args_field_text = {""};
				string[] args_more_title = {""};
				string[,] args_formulas = {{"",""}};
				string[,] args_width = {{"",""}};
		...
		void on_button_busca_producto_clicked (object sender, EventArgs args)
		{
			...
			if(button_buscarproducto.Name == "button_busca_producto"){
				...
				label_desc_proveedor.Text = "Descrp. de Producto";
				entry_producto_proveedor.IsEditable = false;
				label_codprod_proveedor.Text = "Codigo de Producto";
				entry_codprod_proveedor.IsEditable = false;
				label3.Hide();
				entry_lote.Hide();
				label4.Hide();
		...
		void on_selecciona_producto_clicked (object sender, EventArgs args)
		{
					...
					// Llenando el TreeView para la requisicion
		 			if (lista_de_producto.Selection.GetSelected(out model, out iterSelected)){
						if(activovalidprodrequi == true){
							nrorequisicion_prod = (string) classpublic.lee_registro_de_tabla ("osiris_erp_requisicion_deta", "id_requisicion", "WHERE id_producto = '" + model.GetValue (iterSelected, 0).ToString().Trim() + "' AND (comprado = 'false' AND liberado_compra = 'false' AND eliminado = 'false' AND no_autorizada = 'false' AND no_recibido_no_facturado = 'false') ", "id_requisicion", "string"); 			
							if( nrorequisicion_prod == ""){
								...
								MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
									MessageType.Error,ButtonsType.Close, "Este producto no se puede requisar NO se ha comprado aun, verique en el departamento de COMPRAS... REQ. "+nrorequisicion_prod.Trim());
								msgBoxError.Run ();		msgBoxError.Destroy();
							}
						}else{
							contador_items_requisados += 1;
 		...
 		void on_button_autorizar_compra_clicked(object sender, EventArgs args)
 		{
			...
			msgBox.Destroy();
		 	if (miResultado == ResponseType.Yes){
		 		if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_autoriza_reqcompra","WHERE acceso_autoriza_reqcompra = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_autoriza_reqcompra","bool") == "True"){
					NpgsqlConnection conexion;
					conexion = new NpgsqlConnection (connectionString+nombrebd);
					try{
						conexion.Open ();
		 	...
		 	if (miResultado == ResponseType.Yes){
				...
				try{
					...
					comando.CommandText = "UPDATE osiris_erp_requisicion_enca SET enviada_a_compras = 'true',"+
								...
								"sub_total_req = '" +entry_sub_total.Text.Trim()+"',"+
								"iva_req = '" +entry_iva.Text.Trim()+"',"+
								"total_req = '"+entry_total.Text.Trim()+"' "+							
								"WHERE id_requisicion = '"+this.idrequisicion.ToString()+"';";																	
					//Console.WriteLine(comando.CommandText);
					...
					if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_envio_reqautoriza","WHERE acceso_envio_reqautoriza = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_envio_reqautoriza","bool") == "True"){
					...
					if(classpublic.enviar_email == true){
						...
						mailreceive = "mayela.mendez@medicanoresteion.com.mx";
		...
		void on_button_no_autoriza_prod_clicked(object sender, EventArgs args)
 		{
			if((string) classpublic.lee_registro_de_tabla("osiris_empleado","acceso_noautoriza_prodreq","WHERE acceso_noautoriza_prodreq = 'true' AND login_empleado = '"+LoginEmpleado+"' ","acceso_noautoriza_prodreq","bool") == "True"){
				TreeModel model;
				TreeIter iterSelected;
				if (lista_requisicion_productos.Selection.GetSelected(out model, out iterSelected)){			
					MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
					...
					msgBox.Destroy();
				 	if (miResultado == ResponseType.Yes){
						...
						try{
							...
							comando.ExecuteNonQuery();					comando.Dispose();
							if(classpublic.enviar_email == true){
								string mensaje_email = (string) lista_requisicion_productos.Model.GetValue (iterSelected,1)+"<br>" +
		...
		void on_button_selecciona_requisicion_clicked(object sender, EventArgs args)
		{
 		...
 		void llenado_de_requisicion()
 		{
			...
			subtotal_requisicion = 0;
			ivatotal_requisicion = 0;
			totaltotal_requisicion = 0;
			button_autorizar_compra.Sensitive = false;
			idtipointernamiento = idcentrocosto;
			...
			try{
				...
				if ((bool) lector.Read()){
					...
					if (centrocosto == "COMPRAS" && accesomodulo == 0){
						...
						idtipointernamiento = (int) lector["id_tipo_admisiones"];
						idtipointernamiento2 = (int) lector["id_tipo_admisiones_cargada"];
						...
						comando.CommandText  = "SELECT id_requisicion,to_char(osiris_erp_requisicion_deta.id_producto,'999999999999') AS idproducto,"+
										...
										"osiris_productos.nombre_articulo AS nombrearticulo,to_char(osiris_productos.cantidad_de_embalaje,'9999.99') AS cantidadembalaje,osiris_productos.aplicar_iva,"+
						...
						while ((bool) lector1.Read()){
							calculodeiva = 0;
							contador_items_requisados += 1;
							if ((bool) lector1["aplicar_iva"]){
								calculodeiva = ((float.Parse((string) lector1["costoproducto"]) * valoriva)/100) * float.Parse((string) lector1["cantidadsolicitada"]);
							}
							...
							subtotal_requisicion += float.Parse((string) lector1["costoproducto"]) * float.Parse((string) lector1["cantidadsolicitada"]);
							ivatotal_requisicion += calculodeiva;
							nombre_proveedor1 = lector1["descripcion_proveedor"].ToString().Trim();
							nombre_proveedor2 = lector1["descripcion_proveedor2"].ToString().Trim();
							nombre_proveedor3 = lector1["descripcion_proveedor3"].ToString().Trim();							
							treeViewEngineRequisicion.AppendValues((string) lector1["cantidadsolicitada"],
														(string) lector1["nombrearticulo"], 
						...
						totaltotal_requisicion = subtotal_requisicion + ivatotal_requisicion;
						if(autorizaparacompra == true){
							button_autorizar_compra.Sensitive = activar_botton_autoriza;
						}
						entry_totalitems_productos.Text = contador_items_requisados.ToString().Trim();
						entry_sub_total.Text = subtotal_requisicion.ToString("F");
						entry_iva.Text = ivatotal_requisicion.ToString("F");
						entry_total.Text = totaltotal_requisicion.ToString("F");
    				}else{
    					if ( idcentrocosto == (int) lector["id_tipo_admisiones"]){
							...
							comando.CommandText = "SELECT id_requisicion,to_char(osiris_erp_requisicion_deta.id_producto,'999999999999') AS idproducto,"+
										"to_char(cantidad_solicitada,'999999.99') AS cantidadsolicitada,comprado,cantidad_recibida,"+
										"osiris_productos.nombre_articulo AS nombrearticulo,to_char(osiris_productos.cantidad_de_embalaje,'9999.99') AS cantidadembalaje,aplicar_iva,"+
										"osiris_productos.tipo_unidad_producto,to_char(numero_orden_compra,'9999999999') AS numeroordencompra,"+
										"autorizada,to_char(fechahora_autorizado,'yyyy-MM-dd') AS fechahoraautorizado,"+
		...
		// llena la lista de productos
 		...
 		void llenando_lista_de_productos()
 		{
			...
			if(radiobutton_nombre.Active == true) {query_tipo_busqueda = "AND osiris_productos.nombre_articulo LIKE '%"+entry_expresion.Text.ToUpper().Trim()+"%' ORDER BY nombre_articulo; "; }
			...
			try{
				...
				comando = conexion.CreateCommand ();
               	comando.CommandText = "SELECT to_char(osiris_productos.id_producto,'999999999999') AS codProducto,"+
							"osiris_productos.nombre_articulo AS nombrearticulo,osiris_productos.nombre_articulo,osiris_productos.nombre_generico_articulo, "+
		...
		void crea_treeview_requisicion()
		{
			treeViewEngineRequisicion = new TreeStore(typeof(string), 	// Camdidad de unidades solicitadas en compras
			                                        ...
			                                        typeof(bool),		// indica si el producto tiene iva
			                                        typeof(string));	// 30
		...
		void on_packproductos_clicked(object sender, EventArgs args)
		{
			...
			if (lista_de_producto.Selection.GetSelected(out model, out iterSelected)){
				...
				entry_codprod_proveedor.Text = (string) model.GetValue(iterSelected, 0);
				entry_producto_proveedor.Text = (string) model.GetValue(iterSelected, 1);
			}
		}
