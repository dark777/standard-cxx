...
TEST(RangeIntegerTests, ValueAnotherStepWrongDirectionExceptionTest)
{
 ...
 try{
    ...
    {
     thrown = true;
     EXPECT_EQ(std::string{"Range arguments must result in termination"}, std::string{e.what()});
    }
    EXPECT_TRUE(thrown);
}
...
TEST(RangeIntegerTests, ValueZeroStepExceptionTest)
{
 ...
 try{
    ...
    {
     thrown = true;
     EXPECT_EQ(std::string{"Range step argument must not be zero"}, std::string{e.what()});
    }
    EXPECT_TRUE(thrown);
}
...
TEST(RangeIntegerTests, IntegerWrapAroundReverseTest)
{
 bool thrown = false;
 
 try{
    range(int64_t{1}, std::numeric_limits<int64_t>::max(), int64_t{-1});
    }
    catch(const std::invalid_argument& e)
    {
     thrown = true;
...
{
 auto testRange = range(std::numeric_limits<int64_t>::min() + 1, std::numeric_limits<int64_t>::min(), int64_t{-2});
...
{
 auto testRange = range(std::numeric_limits<int64_t>::max() - 1, std::numeric_limits<int64_t>::max(), int64_t{2});
 unsigned int loopCount = 0;
